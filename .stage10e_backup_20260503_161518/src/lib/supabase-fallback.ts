...
export async function syncGoogleCalendarInboundFromSupabase() {
  // GOOGLE_CALENDAR_STAGE10B_CLIENT_INBOUND_SYNC_API
  return callApi<{ ok: boolean; imported?: number; updated?: number; deleted?: number; skipped?: number; conflictCount?: number; conflicts?: Record<string, unknown>[] }>('/api/google-calendar?route=sync-inbound', { method: 'POST', body: JSON.stringify({}) });
}
...
export async function syncGoogleCalendarInboundFromSupabase(input?: { updatedMin?: string | null }) {
  // GOOGLE_CALENDAR_STAGE10D_CLIENT_INBOUND_SYNC_HELPER
  return callApi<GoogleCalendarInboundSyncResult>('/api/google-calendar?action=sync-inbound', {
    method: 'POST',
    body: JSON.stringify({ action: 'sync-inbound', updatedMin: input?.updatedMin || null }),
  });
}
