...
import {
  ...
  syncGoogleCalendarInboundFromSupabase,
...
export default function Calendar() {
  ...
  const inboundGoogleSyncRef = useRef(false);
  ...
  async function syncGoogleCalendarInboundBeforeCalendarLoad() {
    // GOOGLE_CALENDAR_STAGE10D_CALENDAR_AUTO_PULL_SYNC
    if (inboundGoogleSyncRef.current) return null;
    inboundGoogleSyncRef.current = true;
    try {
      const result = await syncGoogleCalendarInboundFromSupabase();
      const conflicts = Array.isArray(result?.conflicts) ? result.conflicts : [];
      const changed = Number(result?.created || 0) + Number(result?.updated || 0) + Number(result?.deleted || 0);
      if (conflicts.length) {
        const first = conflicts[0] || {};
        const title = String((first as any).googleTitle || (first as any).title || 'Wydarzenie z Google Calendar');
        toast.warning('Konflikt terminu z Google Calendar', {
          description: `Import z Google nakłada się z istniejącym wpisem. Pierwszy konflikt: ${title}.`,
        });
      }
      if (changed > 0 && !conflicts.length) {
        toast.success(`Google Calendar zsynchronizowany: ${changed} zmian`);
      }
      return result;
    } catch (error: any) {
      console.warn('GOOGLE_CALENDAR_STAGE10D_INBOUND_AUTO_PULL_FAILED', error?.message || error);
      return null;
    } finally {
      inboundGoogleSyncRef.current = false;
    }
  }
  ...
  useEffect(() => {
    ...
    const loadBundle = async () => {
      try {
        setLoading(true);
        const inboundSync = await syncGoogleCalendarInboundFromSupabase().catch((error: any) => {
          console.warn('GOOGLE_CALENDAR_INBOUND_SYNC_FAILED', error?.message || error);
          return null;
        });
        // GOOGLE_CALENDAR_STAGE10B_CALENDAR_LOAD_INBOUND_SYNC
        if (!cancelled && inboundSync?.conflictCount) {
          toast.warning(`Google Calendar: wykryto ${inboundSync.conflictCount} konfliktów terminów po synchronizacji.`);
        }
        const [bundle, caseRows, clientRows] = await Promise.all([
          fetchCalendarBundleFromSupabase(),
          fetchCasesFromSupabase(),
