/* CLOSEFLOW_STAGE158_OVERLAY_PORTAL_DENSITY_PATCHER */
const fs = require('fs');
const path = require('path');

const repo = process.argv[2];
if (!repo) throw new Error('Usage: node apply-stage158-overlay-portal-density.cjs <repo>');
...
const importLine = "import './styles/closeflow-overlay-portal-density-stage158.css';";
if (!app.includes(importLine)) {
  ...
  app = lines.join('\n');
  console.log('UPDATED src/App.tsx: added Stage158 overlay portal density CSS import');
} else {
  console.log('SKIPPED src/App.tsx: Stage158 import already present');
}

write(rel, app);
console.log('DONE Stage158 overlay portal density patcher.');
