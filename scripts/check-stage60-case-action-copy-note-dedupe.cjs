...
notContains('src/pages/CaseDetail.tsx', 'data-case-create-action="note"', 'duplicate main note action button');
...
contains('scripts/check-stage57-case-create-action-hub.cjs', 'duplicate note action button removed by Stage60', 'Stage57 guard updated');
...
contains('package.json', 'check:stage59-case-note-follow-up-prompt && npm.cmd run check:stage60-case-action-copy-note-dedupe && npm.cmd run verify:client-detail-operational-ui', 'Stage60 included in case operational verify');
