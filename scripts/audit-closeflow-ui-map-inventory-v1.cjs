...
function getLine(lines, n) {
  return lines[n - 1] || '';
}

function compactSnippet(lines, start, len = 8) {
  return lines.slice(Math.max(0, start - 1), Math.min(lines.length, start - 1 + len)).join('\n').trim();
}
...
function parseLucideImports(text) {
  ...
  while ((m = re.exec(text))) {
    ...
    for (const raw of block.split(',')) {
      ...
      if (/^[A-Za-z][A-Za-z0-9_]*$/.test(local)) result.push({ imported, local, importLine: lineOf(text, m.index) });
    }
  }
  return result;
}
...
function roleForIcon(name) {
  ...
  if (/file|note|text/.test(n)) return 'note';
  if (/calendar|event/.test(n)) return 'event';
  if (/clock|timer/.test(n)) return 'time';
  if (/check|circlecheck|checkcircle|task|list/.test(n)) return 'task_status';
  ...
  if (/alert|warning|triangle/.test(n)) return 'risk_alert';
  ...
  if (/arrow/.test(n)) return 'navigation';
  return 'unclassified';
}
...
for (const file of files) {
  ...
  for (const icon of lucideImports) {
    const usageRe = new RegExp(`<${icon.local}(?:\\s|>|/)`, 'g');
    const usages = [];
    let m;
    while ((m = usageRe.exec(text))) {
      usages.push({ line: lineOf(text, m.index), snippet: getLine(lines, lineOf(text, m.index)).trim() });
    }
  ...
  while ((statMatch = statRe.exec(text))) {
    ...
    inventory.metricTileUsages.push({
      file: relative,
      line: startLine,
      component: 'StatShortcutCard',
      snippet: compactSnippet(lines, startLine, 12),
    });
  }
