...
check(clients.includes('Zarobione Ĺ‚Ä…cznie'), 'ClientListCard must render lifetime earned label');
...
check(clients.includes('commissionPaidAmount'), 'Zarobione Ĺ‚Ä…cznie must be based on paid commission amount');
...
check(dict.includes('Zarobione Ĺ‚Ä…cznie = suma wpĹ‚aconej prowizji ze wszystkich spraw klienta.'), 'UI Dictionary must define lifetime earned source');
check(dict.includes('Nie pokazywaÄ‡ â€žLeadyâ€ť'), 'UI Dictionary must ban Leady from client card');
check(dict.includes('Nie uĹĽywaÄ‡ badge â€žAktywna sprawaâ€ť'), 'UI Dictionary must ban Aktywna sprawa badge from client card');
...
for (const rel of touchedFiles) {
  ...
  check(!mojibake.test(content), `Mojibake detected in touched file: ${rel}`);
}
