...
if (!text.includes('STAGE71_CLIENTS_LEADS_ONLY_ATTENTION')) fail('missing stage marker');
if (!text.includes('const followupLeadCandidates = useMemo')) fail('missing lead-only candidates source');
...
if (!text.includes('Leady do uwagi')) fail('missing lead-only rail title');
...
if (!text.includes('followupLeadCandidates.length ? followupLeadCandidates.map')) fail('rail does not render lead candidates');
