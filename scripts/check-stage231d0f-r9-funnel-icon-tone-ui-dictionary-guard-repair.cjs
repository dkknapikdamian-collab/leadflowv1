...
if (failures.length) {
  console.error('STAGE231D0F-R9 Funnel icon tone UI Dictionary guard repair guard: FAIL');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('STAGE231D0F-R9 Funnel icon tone UI Dictionary guard repair guard: PASS');
