...
if (!repo) throw new Error('Usage: node apply-stage162-cf-modal-surface-lower-smaller.cjs <repo>');

const appPath = path.join(repo, 'src/App.tsx');
let app = fs.readFileSync(appPath, 'utf8');

const importLine = "import './styles/closeflow-cf-modal-surface-lower-smaller-stage162.css';";
if (!app.includes(importLine)) {
  ...
  console.log('UPDATED src/App.tsx: added Stage162 CSS import');
} else {
  console.log('SKIPPED src/App.tsx: Stage162 import already present');
}
