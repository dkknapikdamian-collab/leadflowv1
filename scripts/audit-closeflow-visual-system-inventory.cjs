...
/*
CLOSEFLOW_VISUAL_SYSTEM_INVENTORY_STAGE0_FINALIZER
Inventory current visual-system drift before visual migration.
This script is read-only except --write generated markdown/json.
*/
const fs = require('fs');
...
const ACTIVE_PAGE_ALLOWLIST = [
  'TodayStable', 'Today', 'Leads', 'LeadDetail', 'Clients', 'ClientDetail', 'Cases', 'CaseDetail',
  'Tasks', 'TasksStable', 'Calendar', 'AiDrafts', 'Activity', 'Billing', 'Settings',
  'NotificationsCenter', 'SupportCenter', 'ResponseTemplates', 'Templates', 'AdminFeedback',
  'Admin', 'AdminAiSettings', 'ClientPortal', 'Login', 'UiPreviewVNext', 'UiPreviewVNextFull',
];

const STANDARD_WRAPPER_MARKERS = [
  '<Layout', "from '../components/Layout'", 'from "../components/Layout"', 'data-closeflow-page-wrapper',
  'closeflow-page-shell', 'CloseflowPageShell', 'PageShell', 'main-page-shell', 'operator-shell', 'app-shell',
];
const STANDARD_TILE_MARKERS = [
  'StatShortcutCard', 'data-today-tile-card', 'eliteflow-metric', 'metric-tile', 'closeflow-metric',
  'UnifiedMetric', 'MetricCard', 'StatsCard', 'StatCard',
];
const STANDARD_HERO_MARKERS = [
  'data-page-hero', 'closeflow-page-hero', 'UnifiedPageHead', 'PageHero', 'PageHeader', 'stage37', 'page-head',
];
...
function decisionForCssImport(importPath) {
  ...
  if (/case-detail-simplified/.test(importPath)) return 'legacy';
  if (/eliteflow|visual-stage|hotfix|stage\d|stage[A-Z]\d|stage[A-Z]|case-detail-stage2/.test(importPath)) return 'migrujemy';
  return 'migrujemy';
}
function decisionForCssFile(file) {
  ...
  if (/case-detail-simplified/.test(name)) return 'legacy';
...
function fileNameNoExt(file) { return path.basename(file).replace(/\.(tsx|jsx|ts|js)$/, ''); }
function resolveActivePages() {
  const appText = read('src/App.tsx') + '\n' + read('src/main.tsx');
  const pageFiles = walk('src/pages').filter((p) => /\.(tsx|jsx)$/.test(p)).map(rel).sort();
  const active = [];
  const inactive = [];
...
function classifyActiveScreen(file) {
  ...
  return {
    ...
    missing: [!wrapper ? 'standard wrapper' : null, !tiles ? 'standard tiles' : null, !hero ? 'standard page hero' : null].filter(Boolean),
...
function buildInventory() {
  ...
  while ((m = importRegex.exec(indexCssText))) {
    cssImports.push({ kind: 'src/index.css import', file: 'src/index.css', line: lineNo(indexCssText, m.index), import: m[1], decision: decisionForCssImport(m[1]) });
  }
  ...
  const namedCssFamilies = styleFiles
    ...
    .map((file) => ({
      kind: 'named CSS family', file,
      family: /visual-stage/i.test(file) ? 'visual-stage*' : /hotfix-/i.test(file) ? 'hotfix-*' : /eliteflow-/i.test(file) ? 'eliteflow-*' : /stage.*\.css/i.test(file) ? 'stage*.css' : 'other',
      decision: decisionForCssFile(file),
    }));
  const sourceFiles = [...walk('src/pages'), ...walk('src/components')].filter((p) => /\.(tsx|jsx|ts|js)$/.test(p)).map(rel).sort();
  let localTiles = [];
  let localHeaders = [];
  let localRows = [];
  let localForms = [];
  ...
  for (const file of sourceFiles) {
    const text = read(file);
    localTiles.push(...collectMatches(file, /\bfunction\s+[A-Z][A-Za-z0-9]*(?:Tile|Card|Metric|Stat)[A-Za-z0-9]*\b/g, 'local tile/card component', decisionForLocalSurface));
    localTiles.push(...collectMatches(file, /\bconst\s+[A-Z][A-Za-z0-9]*(?:Tile|Card|Metric|Stat)[A-Za-z0-9]*\s*=/g, 'local tile/card component', decisionForLocalSurface));
    localHeaders.push(...collectMatches(file, /\bfunction\s+[A-Z][A-Za-z0-9]*(?:Header|Hero|PageHead)[A-Za-z0-9]*\b/g, 'local page header/hero', decisionForLocalSurface));
    localHeaders.push(...collectMatches(file, /\bconst\s+[A-Z][A-Za-z0-9]*(?:Header|Hero|PageHead)[A-Za-z0-9]*\s*=/g, 'local page header/hero', decisionForLocalSurface));
    localRows.push(...collectMatches(file, /\bfunction\s+[A-Z][A-Za-z0-9]*(?:Row|ListItem|Entry)[A-Za-z0-9]*\b/g, 'local list row', decisionForLocalSurface));
    localRows.push(...collectMatches(file, /\bconst\s+[A-Z][A-Za-z0-9]*(?:Row|ListItem|Entry)[A-Za-z0-9]*\s*=/g, 'local list row', decisionForLocalSurface));
    localForms.push(...collectMatches(file, /<(form|Form)\b/g, 'local form surface', decisionForLocalSurface));
    localForms.push(...collectMatches(file, /\bfunction\s+[A-Z][A-Za-z0-9]*(?:Form)[A-Za-z0-9]*\b/g, 'local form component', decisionForLocalSurface));
    localForms.push(...collectMatches(file, /\bconst\s+[A-Z][A-Za-z0-9]*(?:Form)[A-Za-z0-9]*\s*=/g, 'local form component', decisionForLocalSurface));
    localRightCards.push(...collectMatches(file, /\b(function|const)\s+[A-Z][A-Za-z0-9]*(?:RightCard|RightRail|SideCard|AsideCard)[A-Za-z0-9]*/g, 'local right-card/right-rail surface', decisionForLocalSurface));
    localRightCards.push(...collectMatches(file, /className=[{]?['"][^'"]*(?:right-card|right-rail|side-card|aside-card)[^'"]*['"]/gi, 'local right-card/right-rail class', decisionForLocalSurface));
    localModals.push(...collectMatches(file, /\bfunction\s+[A-Z][A-Za-z0-9]*(?:Dialog|Modal)[A-Za-z0-9]*\b/g, 'local modal/dialog component', decisionForLocalSurface));
    localModals.push(...collectMatches(file, /\bconst\s+[A-Z][A-Za-z0-9]*(?:Dialog|Modal)[A-Za-z0-9]*\s*=/g, 'local modal/dialog component', decisionForLocalSurface));
    localModals.push(...collectMatches(file, /<(Dialog|DialogContent|Modal)\b/g, 'local modal/dialog usage', decisionForLocalSurface));
    if (text.includes('LOCAL_VISUAL_SYSTEM_EXCEPTION')) {
      localHeaders.push({ kind: 'local visual system exception marker', file, line: 1, match: 'LOCAL_VISUAL_SYSTEM_EXCEPTION', decision: 'legacy' });
    }
  }
  localTiles = uniqBy(localTiles, (x) => `${x.file}:${x.line}:${x.match}`);
  localHeaders = uniqBy(localHeaders, (x) => `${x.file}:${x.line}:${x.match}`);
  localRows = uniqBy(localRows, (x) => `${x.file}:${x.line}:${x.match}`);
  localForms = uniqBy(localForms, (x) => `${x.file}:${x.line}:${x.match}`);
  localRightCards = uniqBy(localRightCards, (x) => `${x.file}:${x.line}:${x.match}`);
  localModals = uniqBy(localModals, (x) => `${x.file}:${x.line}:${x.match}`);
  ...
  return {
    ...
    summary: {
      ...
      localRightCards: localRightCards.length,
      localModals: localModals.length,
...
| Lokalne formularze | ${inv.summary.localForms} |
| Lokalne right-card / right-rail | ${inv.summary.localRightCards} |
...
${tableRows(surfaceRows(inv.localSurfaces.localTiles), ['file', 'line', 'match', 'decision'], 120)}

## Lokalne page headery / hero

${tableRows(surfaceRows(inv.localSurfaces.localPageHeaders), ['file', 'line', 'match', 'decision'], 120)}

## Lokalne list rows / entries

${tableRows(surfaceRows(inv.localSurfaces.localListRows), ['file', 'line', 'match', 'decision'], 120)}

## Lokalne formularze

${tableRows(surfaceRows(inv.localSurfaces.localForms), ['file', 'line', 'match', 'decision'], 120)}

## Lokalne right-card / right-rail

${tableRows(surfaceRows(inv.localSurfaces.localRightCards), ['file', 'line', 'match', 'decision'], 120)}

## Lokalne modale / dialogi

${tableRows(surfaceRows(inv.localSurfaces.localModals), ['file', 'line', 'match', 'decision'], 160)}
