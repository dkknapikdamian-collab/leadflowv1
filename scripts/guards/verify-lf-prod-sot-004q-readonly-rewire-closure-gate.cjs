...
if (fs.readdirSync(rel('_project/runs')).some((n) => n.includes('LF-PROD-SOT-004R'))) fail('004R report exists')
let changed=[]; try { changed=childProcess.execSync('git diff --name-only HEAD',{encoding:'utf8'}).trim().split(/\r?\n/).filter(Boolean) } catch (_) {}
const allowedChanged = new Set(['package.json', adapterRel, reportRel, testRel, 'scripts/guards/verify-lf-prod-sot-004q-readonly-rewire-closure-gate.cjs', 'scripts/guards/verify-lf-prod-sot-004m-today-runtime-import-smoke-and-decision.cjs', 'scripts/guards/verify-lf-prod-sot-004n-tasks-status-date-readonly-runtime-import.cjs', 'scripts/guards/verify-lf-prod-sot-004o-calendar-status-date-readonly-runtime-import.cjs', 'scripts/guards/verify-lf-prod-sot-004p-list-cards-status-date-readonly-runtime-import.cjs'])
