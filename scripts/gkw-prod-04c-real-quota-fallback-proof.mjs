...
const { runQuotaFallbackProof } = require('../runtime/gkw-provider-real-proof');
...
function usage() {
  return `GKW-PROD-04C-R2 real quota/fallback proof

Real probe:
  node scripts/gkw-prod-04c-real-quota-fallback-proof.mjs --provider google_ai_studio --real-probe --owner-approved --max-estimated-cost 5

Quota burn / fallback proof:
  node scripts/gkw-prod-04c-real-quota-fallback-proof.mjs --provider google_ai_studio --quota-burn --fallback-required --owner-approved --max-attempts 20 --max-estimated-cost 5

Rules:
  - controlled skip is not quota exhausted evidence
  - quota unknown stays unknown unless provider/local state gives remaining limits
  - fallback proof requires a real second provider/model/account call
  - raw secrets are never printed or written
`;
}
