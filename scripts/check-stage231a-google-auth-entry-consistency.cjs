...
function hasGoogleHandler(source, intent) {
  return source.includes('onClick={handleGoogleLogin}')
    || source.includes("handleGoogleLogin('" + intent + "')")
    || source.includes('handleGoogleLogin("' + intent + '")');
}
...
if (!hasGoogleHandler(loginTab, 'login')) fail('Login tab Google handler missing');
...
if (!hasGoogleHandler(registerTab, 'register')) fail('Register Google handler missing');
