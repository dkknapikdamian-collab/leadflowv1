...
if (failures.length) {
  console.error('STAGE231D0F-R10 Funnel icon tone PowerShell StrictMode repair guard: FAIL');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('STAGE231D0F-R10 Funnel icon tone PowerShell StrictMode repair guard: PASS');
