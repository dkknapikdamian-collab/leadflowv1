...
if (!repo) throw new Error('Usage: node apply-stage165-modal-unified-event-motif.cjs <repo>');

const appPath = path.join(repo, 'src/App.tsx');
let app = fs.readFileSync(appPath, 'utf8');

const importLine = "import './styles/closeflow-modal-unified-event-motif-source-truth-stage165.css';";
if (!app.includes(importLine)) {
  ...
  console.log('UPDATED src/App.tsx: added Stage165 unified modal source truth CSS import');
} else {
  console.log('SKIPPED src/App.tsx: Stage165 import already present');
}
