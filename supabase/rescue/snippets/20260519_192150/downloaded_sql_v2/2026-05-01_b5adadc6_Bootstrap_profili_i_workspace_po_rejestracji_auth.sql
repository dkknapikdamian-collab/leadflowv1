...
-- V1: jeden uĹĽytkownik = jeden workspace. workspace_members zostaje technicznie pod przyszĹ‚oĹ›Ä‡.
...
create table if not exists public.workspaces (
  ...
  name text not null default 'MĂłj Workspace',
...
alter table if exists public.workspaces add column if not exists name text not null default 'MĂłj Workspace';
...
declare
  ...
  display_name text;
begin
  display_name := coalesce(new.raw_user_meta_data->>'full_name', new.raw_user_meta_data->>'name', new.email, 'UĹĽytkownik');
  ...
  insert into public.workspaces (
    ...
    new.id,
    new.id,
    coalesce(nullif(display_name, ''), 'MĂłj') || ' Workspace',
    'trial_21d',
    'trial_active',
