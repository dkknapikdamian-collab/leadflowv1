...
-- Supabase workspace context repair v3
-- Cel: naprawić błąd brakujących kolumn profiles.workspace_id, profiles.id oraz NOT NULL workspaces.owner_user_id
...
-- 2b. Existing workspaces may have been created by older scripts.
-- If owner_user_id is NOT NULL in the current database, every insert must provide it.
-- This repair also fills old null values where the column allows them.
do $$
begin
  if pg_temp.cf_column_exists('workspaces', 'owner_user_id') then
    execute 'update public.workspaces set owner_user_id = gen_random_uuid(), updated_at = now() where owner_user_id is null';
  end if;

  if pg_temp.cf_column_exists('workspaces', 'owner_id') and pg_temp.cf_column_exists('workspaces', 'owner_user_id') then
    execute 'update public.workspaces set owner_id = owner_user_id, updated_at = now() where owner_id is null and owner_user_id is not null';
  end if;

  if pg_temp.cf_column_exists('workspaces', 'created_by_user_id') and pg_temp.cf_column_exists('workspaces', 'owner_user_id') then
    execute 'update public.workspaces set created_by_user_id = owner_user_id, updated_at = now() where created_by_user_id is null and owner_user_id is not null';
  end if;
end $$;
...
insert into workspace_repair_seed(profile_id, workspace_id, workspace_name, owner_user_id)
select
  ...
  coalesce(
    case when p.id::text ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then p.id::uuid else null end,
    case when coalesce(p.firebase_uid, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then p.firebase_uid::uuid else null end,
    case when coalesce(p.auth_uid, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then p.auth_uid::uuid else null end,
    case when coalesce(p.external_auth_uid, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then p.external_auth_uid::uuid else null end,
    gen_random_uuid()
  ) as owner_user_id
