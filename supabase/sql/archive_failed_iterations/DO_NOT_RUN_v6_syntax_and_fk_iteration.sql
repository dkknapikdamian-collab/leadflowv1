...
-- Workspace context repair v6 FK-target-safe.
-- Run in Supabase SQL Editor as one query.
-- Goal: repair workspace context without violating workspaces.owner_user_id FK.

begin;

create extension if not exists pgcrypto;

-- Keep base public.users available when this project uses public users.
...
-- Ensure profiles have a stable text id.
...
-- Business table compatibility columns.
do $$
begin
...
-- Detect exact FK target for workspaces.owner_user_id.
drop table if exists pg_temp.cf_owner_fk_target;
create temporary table cf_owner_fk_target as
select
...
insert into cf_owner_fk_target(ref_schema, ref_table, ref_column)
select 'public', 'users', 'id'
where not exists (select 1 from cf_owner_fk_target);
...
select
  ...
  coalesce(
    ...
    gen_random_uuid()
  ) as planned_public_user_id,
  coalesce(nullif(trim(p.full_name), ''), nullif(trim(p.email), ''), concat('Workspace ', left(p.id::text, 8))) as workspace_name
from public.profiles p;

-- Final owner map. owner_user_id is set only when the owner exists in the FK target table.
drop table if exists pg_temp.cf_profile_owner_map;
create temporary table cf_profile_owner_map (
  profile_id text not null,
  email text null,
  full_name text null,
  workspace_id uuid null,
  profile_uuid_candidate uuid null,
  planned_public_user_id uuid not null,
  owner_user_id uuid null,
  workspace_name text not null
);

insert into cf_profile_owner_map(profile_id, email, full_name, workspace_id, profile_uuid_candidate, planned_public_user_id, owner_user_id, workspace_name)
select profile_id, email, full_name, workspace_id, profile_uuid_candidate, planned_public_user_id, null, workspace_name
from cf_profile_seed;

-- If FK targets auth.users, only map to existing auth users. Do not create auth.users manually.
do $$
begin
  if exists (select 1 from cf_owner_fk_target where ref_schema = 'auth' and ref_table = 'users') and to_regclass('auth.users') is not null then
    execute '
      update cf_profile_owner_map m
      set owner_user_id = au.id
      from auth.users au
      where m.owner_user_id is null
        and (
          (m.profile_uuid_candidate is not null and au.id = m.profile_uuid_candidate)
          or (m.email is not null and lower(coalesce(au.email, '''')) = lower(m.email))
        )
    ';
  end if;
end $$;

-- If FK targets public.users, create missing public.users rows only if public.users has no unknown required columns.
do $$
declare
  unknown_required_count integer := 0;
begin
  if exists (select 1 from cf_owner_fk_target where ref_schema = 'public' and ref_table = 'users') then
    ...
    if unknown_required_count = 0 then
      ...
      select
        m.planned_public_user_id,
        m.email,
        m.full_name,
        now(),
        now()
      from cf_profile_owner_map m
      where not exists (
        select 1 from public.users u
        where u.id = m.planned_public_user_id
           or (m.email is not null and lower(coalesce(u.email, '''')) = lower(m.email))
      )
      on conflict (id) do nothing;

      update public.users u
      set email = coalesce(nullif(trim(u.email), ''), m.email),
...
-- The FULL JOIN trick above can be fragile across planners, so run deterministic public.users matching too.
do $$
begin
  if exists (select 1 from cf_owner_fk_target where ref_schema = 'public' and ref_table = 'users') then
    update cf_profile_owner_map m
    ...
    from public.users u
    where m.owner_user_id is null
      and u.id = m.planned_public_user_id;
    ...
    where m.owner_user_id is null
      ...
      and lower(coalesce(u.email, '')) = lower(m.email);
  end if;
end $$;

-- Build owners that definitely exist in the actual FK target.
drop table if exists pg_temp.cf_valid_owner_map;
create temporary table cf_valid_owner_map (
  profile_id text not null,
  email text null,
  full_name text null,
  workspace_id uuid null,
...
begin
  if exists (select 1 from cf_owner_fk_target where ref_schema = 'auth' and ref_table = 'users') and to_regclass('auth.users') is not null then
    execute '
      insert into cf_valid_owner_map(profile_id, email, full_name, workspace_id, owner_user_id, workspace_name)
      select m.profile_id, m.email, m.full_name, m.workspace_id, m.owner_user_id, m.workspace_name
      from cf_profile_owner_map m
      join auth.users u on u.id = m.owner_user_id
      where m.owner_user_id is not null
    ';
  elsif exists (select 1 from cf_owner_fk_target where ref_schema = 'public' and ref_table = 'users') then
    insert into cf_valid_owner_map(profile_id, email, full_name, workspace_id, owner_user_id, workspace_name)
    select m.profile_id, m.email, m.full_name, m.workspace_id, m.owner_user_id, m.workspace_name
    from cf_profile_owner_map m
    join public.users u on u.id = m.owner_user_id
    where m.owner_user_id is not null;
  else
    insert into cf_valid_owner_map(profile_id, email, full_name, workspace_id, owner_user_id, workspace_name)
    select m.profile_id, m.email, m.full_name, m.workspace_id, m.owner_user_id, m.workspace_name
...
-- Backfill profiles.workspace_id from existing memberships.
update public.profiles p
set workspace_id = wm.workspace_id,
    updated_at = now()
from public.workspace_members wm
left join cf_valid_owner_map m on m.profile_id = p.id::text
where p.workspace_id is null
  ...
  and (
    ...
    or (m.owner_user_id is not null and m.owner_user_id::text = wm.user_id::text)
  );

-- Backfill profiles.workspace_id from existing workspace ownership.
update public.profiles p
set workspace_id = w.id,
    updated_at = now()
from cf_valid_owner_map m
join public.workspaces w on w.owner_user_id = m.owner_user_id
where p.workspace_id is null
  and p.id::text = m.profile_id;

-- Create missing workspaces only for owners verified against the actual FK target.
...
select
  ...
  m.workspace_name
from public.profiles p
join cf_valid_owner_map m on m.profile_id = p.id::text
where p.workspace_id is null;
...
insert into cf_repair_summary select 'fk', 'workspaces.owner_user_id.target', concat(ref_schema, '.', ref_table, '.', ref_column) from cf_owner_fk_target;
...
insert into cf_repair_summary select 'profiles', 'resolved_owner_user_match', count(*)::text from cf_valid_owner_map;
insert into cf_repair_summary select 'profiles', 'unresolved_owner_user_match', count(*)::text from cf_profile_owner_map m left join cf_valid_owner_map v on v.profile_id = m.profile_id where v.profile_id is null;
