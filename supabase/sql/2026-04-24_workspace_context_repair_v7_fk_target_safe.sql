...
-- Workspace context repair v7 FK-target-safe.
-- Run in Supabase SQL Editor as one query.
-- Fix over v6: no invalid reference to target alias inside UPDATE ... FROM JOIN ON.
...
-- Important: target alias p is referenced only in WHERE/EXISTS, not in FROM JOIN ON.
