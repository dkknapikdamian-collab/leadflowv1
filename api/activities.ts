...
function normalizeActivity(row: Record<string, unknown>) { return { id: String(row.id || crypto.randomUUID()), caseId: row.case_id ? String(row.case_id) : undefined, leadId: row.lead_id ? String(row.lead_id) : undefined, ownerId: row.owner_id ? String(row.owner_id) : undefined, actorId: row.actor_id ? String(row.actor_id) : undefined, actorType: String(row.actor_type || 'operator'), eventType: String(row.event_type || 'activity'), payload: typeof row.payload === 'object' && row.payload ? row.payload : {}, createdAt: row.created_at || null, updatedAt: row.updated_at || row.updatedAt || row.created_at || null }; }
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      res.status(200).json((result.data || []).map((row: Record<string, unknown>) => normalizeActivity(row))); return;
    }
    if (req.method === 'PATCH') {
      ...
      res.status(200).json(normalizeActivity(updated as Record<string, unknown>)); return;
    }
    ...
    const payload: Record<string, unknown> = { workspace_id: finalWorkspaceId || null, case_id: asNullableUuid(body.caseId), lead_id: asNullableUuid(body.leadId), owner_id: asNullableUuid(body.ownerId), actor_id: asNullableUuid(body.actorId), actor_type: body.actorType || 'operator', event_type: body.eventType || 'activity', payload: body.payload || {}, created_at: new Date().toISOString() };
