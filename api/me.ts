...
function asString(value: unknown, fallback = '') { if (typeof value === 'string') return value; if (value === null || value === undefined) return fallback; return String(value); }
function asNullableString(value: unknown): NullableString { if (typeof value !== 'string') return null; const trimmed = value.trim(); return trimmed ? trimmed : null; }
...
function normalizeProfile(row: Record<string, unknown> | null, fallbackUid: string | null, fallbackEmail: string | null, fallbackFullName: string | null) {
  ...
  return { id: asString(row?.id ?? fallbackUid ?? ''), fullName: asString(row?.full_name ?? row?.fullName ?? fallbackFullName ?? ''), email, role: asString(row?.role ?? (admin ? 'admin' : 'member')), isAdmin: admin };
}
...
function buildAccess(workspace: { subscriptionStatus: string; trialEndsAt: NullableString }) {
  ...
  if (statusRaw === 'paid_active') return { hasAccess: true, status: 'paid_active', isTrialActive: false, isPaidActive: true };
  if (statusRaw === 'trial_ending') return trialFuture ? { hasAccess: true, status: 'trial_ending', isTrialActive: true, isPaidActive: false } : { hasAccess: false, status: 'trial_expired', isTrialActive: false, isPaidActive: false };
  if (statusRaw === 'trial_active') return trialFuture ? { hasAccess: true, status: 'trial_active', isTrialActive: true, isPaidActive: false } : { hasAccess: false, status: 'trial_expired', isTrialActive: false, isPaidActive: false };
...
async function fetchProfile(uid: string | null, email: string | null) {
  if (uid) {
    try { const byId = await selectFirstAvailable([`profiles?id=eq.${encodeURIComponent(uid)}&select=*&limit=1`]); const row = Array.isArray(byId.data) ? byId.data[0] : null; if (row) return row as Record<string, unknown>; } catch {}
  }
  if (email) {
    try { const byEmail = await selectFirstAvailable([`profiles?email=eq.${encodeURIComponent(email)}&select=*&limit=1`]); const row = Array.isArray(byEmail.data) ? byEmail.data[0] : null; if (row) return row as Record<string, unknown>; } catch {}
  }
  return null;
}
...
async function ensureProfile(profileRow: Record<string, unknown> | null, uid: string | null, email: string | null, fullName: string | null, workspaceId: string | null) {
  const nowIso = new Date().toISOString();
  const admin = isAdminEmail(email);
  if (!profileRow) {
    const payload = { id: uid || crypto.randomUUID(), email: email || '', full_name: fullName || '', workspace_id: workspaceId, role: admin ? 'admin' : 'member', is_admin: admin, created_at: nowIso, updated_at: nowIso };
  ...
  const patch: Record<string, unknown> = { updated_at: nowIso };
  let shouldPatch = false;
  if (email && asNullableString(profileRow.email) !== email) { patch.email = email; shouldPatch = true; }
  if (fullName && asNullableString(profileRow.full_name ?? profileRow.fullName) !== fullName) { patch.full_name = fullName; shouldPatch = true; }
  if (workspaceId && asNullableString(profileRow.workspace_id ?? profileRow.workspaceId) !== workspaceId) { patch.workspace_id = workspaceId; shouldPatch = true; }
  if (admin && asNullableString(profileRow.role) !== 'admin') { patch.role = 'admin'; shouldPatch = true; }
  if (admin && !Boolean(profileRow.is_admin ?? profileRow.isAdmin)) { patch.is_admin = true; shouldPatch = true; }
  if (!shouldPatch) return profileRow;
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const resolvedWorkspaceId = uid ? await findWorkspaceId(uid) : null;
