...
export default async function handler(req, res) {
  try {
    ...
    const payload = { workspace_id: finalWorkspaceId, created_by_user_id: body.ownerId || null, name: body.name, company: body.company || '', email: body.email || '', phone: body.phone || '', source, value: amount, partial_payments: normalizePartialPayments(body.partialPayments), summary: '', notes: '', status, priority: isAtRisk ? 'high' : 'medium', is_at_risk: isAtRisk, next_action_title: body.nextStep || '', next_action_at: dueAt, next_action_item_id: null, created_at: nowIso, updated_at: nowIso };
