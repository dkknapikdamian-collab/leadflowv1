...
function asNullableString(value: unknown) { if (typeof value !== 'string') return null; const trimmed = value.trim(); return trimmed ? trimmed : null; }
function asNullableUuid(value: unknown) { const normalized = asNullableString(value); return normalized && isUuid(normalized) ? normalized : null; }
function normalizePartialPayments(value: unknown) { if (!Array.isArray(value)) return []; return value.map((item, index) => { if (!item || typeof item !== 'object') return null; const row = item as Record<string, unknown>; const amount = Number(row.amount || 0); if (!Number.isFinite(amount) || amount < 0) return null; return { id: String(row.id || `payment-${index}`), amount, paidAt: typeof row.paidAt === 'string' && row.paidAt.trim() ? row.paidAt : undefined, createdAt: typeof row.createdAt === 'string' && row.createdAt.trim() ? row.createdAt : new Date().toISOString() }; }).filter(Boolean); }
function normalizeSource(value: unknown) { if (typeof value !== 'string') return 'other'; const normalized = value.trim().toLowerCase(); return SOURCE_ALIASES[normalized as keyof typeof SOURCE_ALIASES] || 'other'; }
function toIsoDateTime(value: unknown) { if (typeof value !== 'string' || !value.trim()) return null; const trimmed = value.trim(); const normalized = /^\d{4}-\d{2}-\d{2}$/.test(trimmed) ? `${trimmed}T09:00:00` : trimmed; const parsed = new Date(normalized); if (Number.isNaN(parsed.getTime())) return null; return parsed.toISOString(); }
function normalizeLead(row: Record<string, unknown>) { return { id: String(row.id || crypto.randomUUID()), name: String(row.name || row.title || row.person_name || ''), company: String(row.company || row.company_name || ''), email: String(row.email || ''), phone: String(row.phone || ''), source: normalizeSource(row.source || row.source_label || row.source_type || 'other'), status: String(row.status || 'new'), nextStep: String(row.next_action_title || row.next_step || row.nextStep || ''), nextActionAt: String(row.next_action_at || row.next_step_due_at || row.nextActionAt || ''), nextActionItemId: String(row.next_action_item_id || row.nextActionItemId || ''), dealValue: Number(row.deal_value || row.value || row.dealValue || 0), partialPayments: normalizePartialPayments(row.partial_payments || row.partialPayments), isAtRisk: Boolean(row.is_at_risk ?? row.isAtRisk ?? (String(row.priority || '').toLowerCase() === 'high')), updatedAt: row.updated_at || row.updatedAt || null }; }
function extractMissingColumn(error: unknown) { const message = error instanceof Error ? error.message : String(error || ''); const match = message.match(/Could not find the '([^']+)' column/i); return match?.[1] || null; }
function omitMissingColumn(payload: Record<string, unknown>, column: string) { const nextPayload = { ...payload }; delete nextPayload[column]; return nextPayload; }
async function insertLeadWithSchemaFallback(payload: Record<string, unknown>) { let currentPayload = { ...payload }; for (let attempt=0; attempt<10; attempt+=1) { try { return await insertWithVariants(['leads'], [currentPayload]); } catch (error) { const missingColumn = extractMissingColumn(error); if (!missingColumn || !OPTIONAL_LEAD_COLUMNS.has(missingColumn) || !(missingColumn in currentPayload)) throw error; currentPayload = omitMissingColumn(currentPayload, missingColumn); } } throw new Error('LEAD_INSERT_SCHEMA_FALLBACK_EXHAUSTED'); }
async function updateLeadWithSchemaFallback(id: string, payload: Record<string, unknown>) { let currentPayload = { ...payload }; for (let attempt=0; attempt<10; attempt+=1) { try { return await updateById('leads', id, currentPayload); } catch (error) { const missingColumn = extractMissingColumn(error); if (!missingColumn || !OPTIONAL_LEAD_COLUMNS.has(missingColumn) || !(missingColumn in currentPayload)) throw error; currentPayload = omitMissingColumn(currentPayload, missingColumn); } } throw new Error('LEAD_UPDATE_SCHEMA_FALLBACK_EXHAUSTED'); }
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      res.status(200).json(normalizeLead(updated as Record<string, unknown>)); return;
    }
    ...
    const payload: Record<string, unknown> = { workspace_id: finalWorkspaceId, created_by_user_id: asNullableUuid(body.ownerId), name: body.name, company: body.company || '', email: body.email || '', phone: body.phone || '', source, value: amount, partial_payments: normalizePartialPayments(body.partialPayments), summary: '', notes: '', status, priority: isAtRisk ? 'high' : 'medium', is_at_risk: isAtRisk, next_action_title: body.nextStep || '', next_action_at: dueAt, next_action_item_id: null, created_at: nowIso, updated_at: nowIso };
