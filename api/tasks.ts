...
function asIsoDate(value: unknown) { if (typeof value !== 'string' || !value.trim()) return null; const parsed = new Date(value); return Number.isNaN(parsed.getTime()) ? null : parsed.toISOString(); }
...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      res.status(200).json((result.data || []).filter((row: Record<string, unknown>) => isTaskRow(row)).map((row: Record<string, unknown>) => normalizeTask(row))); return;
    }
    if (req.method === 'PATCH') {
      ...
      res.status(200).json(normalizeTask(updatedRow as Record<string, unknown>)); return;
    }
