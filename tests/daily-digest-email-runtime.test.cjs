...
test('daily digest cron runs hourly so user-selected hour can work', () => {
  ...
  assert.equal(digestCron.schedule, '5 * * * *');
});
...
test('settings page exposes digest schedule and test-send control', () => {
