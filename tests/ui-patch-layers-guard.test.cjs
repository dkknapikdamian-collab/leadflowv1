...
test('UI patch layer guard encodes base anti-patch contracts', () => {
  ...
  assert.ok(guard.includes('new stacked stage/source-truth CSS imports'));
});

test('UI patch layer guard encodes widened anti-patch policy', () => {
  assert.ok(guard.includes('RAW_PAGE_BUTTON_ALLOWLIST'));
  ...
  assert.ok(guard.includes('raw <button> in src/pages outside explicit debt allowlist'));
  assert.ok(guard.includes('new direct lucide-react imports'));
  assert.ok(guard.includes('new stacked App/global CSS imports'));
  assert.ok(guard.includes('display:none/z-index/!important workarounds'));
  assert.ok(guard.includes('local status/badge color maps'));
