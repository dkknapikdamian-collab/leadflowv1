...
test('R15C runtime keeps note text above follow-up label and warns on linked delete', () => {
  ...
  assert.includes(text, "subtitle: isNoteFollowUpStage231H_R1D2_R10C ? 'Follow-up po notatce'");
  assert.includes(text, 'linkedTaskIdsFromActivitiesStage231H_R1D2_R15C');
  assert.includes(text, 'To jest follow-up przypięty do notatki.');
});