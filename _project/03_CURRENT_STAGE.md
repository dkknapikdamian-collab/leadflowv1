...
## 2026-06-12 15:00 Europe/Warsaw — STAGE231D0F Funnel owner dashboard visual alignment

STATUS: READY_TO_APPLY / UI ALIGNMENT

FAKTY Z REPO:
- `/funnel` jest osobnym widokiem `SalesFunnel`, nie wariantem `/leads`.
- `SalesFunnel.tsx` ma istniejące markery:
  - `data-stage227b-decision-list-view`
  - `data-stage228a-funnel-truth-clickability`
- Lejek ma pozostać listą decyzji właściciela, nie kanbanem.
- Istnieją lokalne komponenty `DecisionTile`, `StagePill`, `Signal`, które w tym etapie są przenoszone do nazw systemowych i wspólnego stylu.

DECYZJE DAMIANA:
- Zamysł lejka jest OK.
- Nie przebudowywać funkcji od zera.
- Dopasować tylko styl i mapę UI.
- Nie ruszać: logiki liczenia, filtrów, routingu, Supabase, leadów, spraw, płatności, SQL, wykresów, drag/drop ani kanbana.

ZMIANA:
- `DecisionTile` -> `FunnelOwnerDecisionTile`
- `StagePill` -> `FunnelStageFilterChip`
- `Signal` -> `FunnelDecisionSignal`
- `DecisionListCard` -> `FunnelDecisionListCard`
- prawy rail -> `FunnelOwnerPriorityRail`
...
RYZYKO:
- Największe ryzyko to przypadkowe zrobienie z lejka kanbana albo wciągnięcie logiki/Supabase w etap wizualny.
- Guard blokuje scope creep: SQL, wykresy, drag/drop i lokalny ciężki styl kafelków.

NASTĘPNY KROK:
- Po PASS i push przejść do następnego UI-baseline albo `STAGE231D0E — CaseDetail checklist tab baseline`, jeśli R4 checklisty jeszcze nie zostały domknięte.
