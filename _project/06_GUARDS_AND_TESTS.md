...
## 2026-05-16 â€” Guard Stage92: calendar selected day readable actions {#STAGE92_CALENDAR_SELECTED_DAY_READABLE_ACTIONS}

Guard: `tests/stage92-calendar-selected-day-readable-actions.test.cjs`

Zakres:
- peĹ‚ne etykiety typĂłw wpisu,
- obecnoĹ›Ä‡ `data-cf-entry-title`,
- peĹ‚ne akcje `Edytuj`, `+1H`, `+1D`, `+1W`, `Zrobione/PrzywrĂłÄ‡`, `UsuĹ„`,
...
## STAGE93_GUARD_CALENDAR_WEEK_RAIL_CLEANUP â€” 2026-05-16
...
- Naprawiono guard Stage93 tak, aby nie szukaĹ‚ kruchego znacznika koĹ„ca po `calendar-week-plan-list`.
- Dodano lokalny sweep: `scripts/check-closeflow-calendar-ui-sweep-stage94.cjs`.
- Sweep wykrywa stare/ryzykowne wzorce Calendar UI: legacy week filter, badge count, dual selected-day render, bĹ‚Ä™dne ĹşrĂłdĹ‚o selected day, duplikaty klas i stare selektory CSS.
...
- `tests/stage96-leads-right-rail-width-position.test.cjs` dopiÄ™ty do quiet release gate.
- Guard blokuje lokalny grid 195px/300px i wymusza wspĂłlny right rail source of truth.
...
## 2026-05-16 Ä‚ËĂ˘â€šÂ¬â€ť Stage104 / Paczka F Ä‚ËĂ˘â€šÂ¬â€ť Calendar loading performance
...
---
## Stage105 / Paczka G Ä‚ËĂ˘â€šÂ¬â€ť Templates delete + visual contract Ä‚ËĂ˘â€šÂ¬â€ť 2026-05-16
...
- `tests/stage98-polish-mojibake-calendar-guard.test.cjs` â€” hard fail for mojibake in `src/`, `tests/`, `scripts/`, `_project/`.
- `tests/stage99-calendar-active-class-contract.test.cjs` â€” active class/CSS contract for Calendar and V9 CSS.
- `tests/stage100-calendar-week-plan-entry-visible.test.cjs` â€” one active week-plan DOM/CSS model and no Stage94 V2/V3/V4 week-plan CSS markers.
- `tests/stage104-calendar-rendered-week-plan-smoke.test.cjs` â€” rendered-skeleton smoke for visible title/relation/actions plus CSS non-hidden checks.
- `scripts/closeflow-release-check-quiet.cjs` â€” includes Stage104 in required tests.
...
- Guard jest podpiÄ™ty do `scripts/closeflow-release-check-quiet.cjs`.
...
Decyzja: guardy UI majÄ… sprawdzaÄ‡ efekt renderu/semi-renderu, nie tylko markery, klasy i stringi w ĹşrĂłdĹ‚ach.

Zakres:
- fixture: Akt jaskiniowiec, 10:29, Wydarzenie, Zaplanowane, Brak powiÄ…zania,
- visible action labels: Edytuj, +1H, +1D, +1W, Zrobione, UsuĹ„,
- brak runtime markerĂłw: ReferenceError, APP_ROUTE_RENDER_FAILED, Missing lazy page export,
- CSS selected-day V9 nie moĹĽe ukrywaÄ‡ krytycznych slotĂłw renderu,
- usuniÄ™ty orphan selector V9 ::after, ktĂłry mĂłgĹ‚ podpinaÄ‡ V9 pod legacy hidden selected-day panel.
...
-
ode --test tests/stage113-closeflow-logo-source-contract.test.cjs pilnuje assetĂłw i mapowania logo w Layout/Login.
...
- Cel: LeadDetail nie moĹĽe mieÄ‡ wĹ‚asnej wyspy `InfoLine` / `lead-detail-contact-grid`; LeadDetail i ClientDetail majÄ… uĹĽywaÄ‡ wspĂłlnego `src/components/entity-contact-card.tsx`.
- Komenda: `node --test tests/stage115-lead-contact-card-client-parity.test.cjs`.
- Test rÄ™czny: /leads/:id, porĂłwnaÄ‡ lewÄ… kartÄ™ kontaktowÄ… z lewÄ… kartÄ… klienta: telefon, e-mail, firma, ostatni kontakt, ikony, przyciski kopiowania.
...
- Purpose: overdue LeadDetail work items must show `ZalegĹ‚e`, use red/danger styling, and remove mojibake separators.
...
## 2026-06-04 â€” Stage221 owner-control roadmap memory guard

Guard:
- `node scripts/check-stage221-owner-control-roadmap-memory.cjs`

Zakres:
- sprawdza, czy `_project/07_NEXT_STEPS.md` zawiera peĹ‚ny blok Stage221,
- sprawdza obecnoĹ›Ä‡ etapĂłw: A35, A35B, A41, A46, A42, A45, A44, A36, A47,
- sprawdza, czy decyzja owner-control zostaĹ‚a dopisana do `_project/04_DECISIONS.md`,
...
Test rÄ™czny:
- PrzeczytaÄ‡ nowy blok w `_project/07_NEXT_STEPS.md` i potwierdziÄ‡, ĹĽe kolejnoĹ›Ä‡ etapĂłw nie skacze po plikach i da siÄ™ z niej wdraĹĽaÄ‡ kolejno.
...
## STAGE226R10D2_DUPLICATE_CONFLICT_CONFIRMATION_GATE_PATCHER_FIX â€” guards
...
- kontrakt: brak .catch(() => ({ candidates: [] })) przy conflict preflight; bĹ‚Ä…d checkerĂłw zatrzymuje zapis; konflikt pokazuje komunikat i wymaga â€žDodaj mimo toâ€ť.

## STAGE226R11_GCAL_TIMEZONE_REMINDER_TRUTH â€” guardy i testy
...
## STAGE226R11B_GCAL_TIMEZONE_TEST_CROSS_REALM_FIX â€” guardy/testy

- data i godzina: 2026-06-06 15:05 Europe/Warsaw
- test: npm run test:stage226r11-gcal-timezone-reminder-truth
- powĂłd: assert z node:assert/strict porĂłwnywaĹ‚ obiekt z VM realm do zwykĹ‚ego obiektu i raportowaĹ‚ false negative mimo identycznej struktury.

<!-- STAGE227A_SALES_FUNNEL_MOVEMENT_VIEW_GUARDS_START -->
## 2026-06-06 15:35 Europe/Warsaw Ä‚ËĂ˘â€šÂ¬â€ť STAGE227A guardy i testy

- `node scripts/check-stage227a-sales-funnel-movement-view.cjs`
- `node --test tests/stage227a-sales-funnel-movement-view.test.cjs`
- `npm run build`
- `npm run verify:closeflow:quiet`
- Regresja R10 lead/client separation, jeĂ„Ä…Ă˘â‚¬Ĺźli pliki istniejÄ‚â€žâ€¦.
- Regresja R11 Google Calendar timezone reminder truth, jeĂ„Ä…Ă˘â‚¬Ĺźli pliki istniejÄ‚â€žâ€¦.
- Manual smoke `/funnel`.
<!-- STAGE227A_SALES_FUNNEL_MOVEMENT_VIEW_GUARDS_END -->

<!-- STAGE227B_SALES_FUNNEL_DECISION_LIST_GUARDS_START -->
## 2026-06-06 15:45 Europe/Warsaw Ä‚ËĂ˘â€šÂ¬â€ť STAGE227B Ä‚ËĂ˘â€šÂ¬â€ť guardy/testy
...
## 2026-06-06 17:05 Europe/Warsaw Ä‚ËĂ˘â€šÂ¬â€ť STAGE228A guardy i testy
...
- Zakres: /funnel bez panelu wĹ‚aĹ›ciciela, kafelki przez OperatorMetricTile, jeden rzÄ…d na desktopie, refresh w stage actions.
<!-- /STAGE228H_R3_GUARD -->

<!-- STAGE228R1_RAIL_TASKS_PATTERN_GUARD -->
## Stage228R1 guard
- npm run check:stage228r1-rail-tasks-pattern
- Chroni klasÄ™ bĹ‚Ä™du: rozjazd tekstu/rytmu right rail wzglÄ™dem wzorca /tasks.
<!-- /STAGE228R1_RAIL_TASKS_PATTERN_GUARD -->
...
Chroni:
- LeadDetail: open-only Braki i blokady + RozwiÄ…ĹĽ brak.
- ClientDetail: open-only Braki i blokady + RozwiÄ…ĹĽ.
- CaseDetail: istniejÄ…cy case_items status accepted/rejected i filtr accepted poza aktywnymi brakami.
...
- Guard nadal wymaga updateTaskInSupabase, status done/resolved i akcji RozwiÄ…ĹĽ/RozwiÄ…ĹĽ brak.
...
- R14 guard akceptuje szczegolowy wording testu:
  - LeadDetail -> Brak -> save -> refresh -> visible -> RozwiÄ…ĹĽ brak -> refresh -> hidden
  - ClientDetail -> Brak -> save -> refresh -> visible -> RozwiÄ…ĹĽ -> refresh -> hidden
...
Kontrakt:
- debug dyktowania jest opcjonalny i domyĹ›lnie wyĹ‚Ä…czony.
...
- `Kopiuj trace` i `WyczyĹ›Ä‡ trace` sÄ… widoczne w panelu trace.
- trace box pokazuje instrukcjÄ™ nawet gdy debug jest wyĹ‚Ä…czony.
...
- Quick capture uĹĽywa lead-form-section / lead-form-textarea jako source truth formularzy.
- Tekst textarea, placeholder, caret i przyciski majÄ… wymuszony kontrast mobilny.
...
- Rejestracja pokazuje copy o moĹĽliwoĹ›ci utworzenia trial/workspace dla nowego konta Google.
- /api/me ma marker obecnego public trial bootstrap przez OAuth.
- Next steps zawiera kolejkÄ™ STAGE231B/C/D/E.
<!-- STAGE231A_GOOGLE_AUTH_ENTRY_CONSISTENCY_GUARD_END -->

## STAGE231D_GOOGLE_AUTH_INTENT_GATE â€” guards/tests
...
## STAGE231D_R5_GOOGLE_LOGIN_MISSING_INTENT_HARD_GATE â€” guards/tests
...
## STAGE231D0A-R3 â€” guard/test closeout
...
Warunek: brak FAIL, brak new blank line at EOF, brak brakujÄ…cych tokenĂłw payloadu Obsidiana.
...
## 2026-06-10 20:05 Europe/Warsaw â€” STAGE231D3-R7 guards
...
- prefiks NajbliĹĽsza akcja: w samym slocie kafelka,
- brak Sprawy:, Aktywna prowizja, Zarobione Ĺ‚Ä…cznie,
- brak osobnych slotĂłw telefonu/e-maila i dwuwierszowego layoutu.
...
## 2026-06-11 Europe/Warsaw â€” STAGE231D0B_R9_R3_GUARD_MOJIBAKE_SELF_SCAN_REPAIR
...
## 2026-06-11 Europe/Warsaw â€” STAGE231D0B_R9_R4_CSS_GUARD_TOKEN_ALIGNMENT
...
## 2026-06-12 15:00 Europe/Warsaw â€” STAGE231D0F guard/test
...
- istniejÄ…ce markery lejka `data-stage227b-decision-list-view` i `data-stage228a-funnel-truth-clickability`,
- wpisy UI Dictionary dla Lejka,
- filtry wĹ‚aĹ›cicielskie,
- zasadÄ™ `lista decyzji / nie kanban`,
- brak SQL, wykresĂłw, drag/drop i ciÄ™ĹĽkiego lokalnego stylu kafelkĂłw.
...
## 2026-06-12 â€” STAGE231D0G-CLOSEOUT Visual Tile Source Truth Atlas
...
Chroni klasÄ™ bĹ‚Ä™du: kafelki display-only bez CTA, brak edycji potencjaĹ‚u, brak widocznego potencjaĹ‚u przy tworzeniu leada, zlewajÄ…cy siÄ™ layout work-row i bĹ‚Ä…d group.key w overflow.

## 2026-06-14 10:40 Europe/Warsaw - STAGE231G R6 guard/test correction

R6 poprawia guard/test po faĹ‚szywym zamkniÄ™ciu R5: walidacja sprawdza realne klasy content/status/actions oraz brak stale group.key tylko w overflow section. Guard jest ASCII/no-BOM.
...
Chroni: potencjaĹ‚ otwierany jako osobny modal tylko do wartoĹ›ci, zapis value + deal_value w API, brak otwierania peĹ‚nej edycji leada z CTA potencjaĹ‚u oraz desktopowe wyrĂłwnanie akcji wiersza.
...
- klasa bÄąâ€šĂ„â„˘du: martwe/ÄąĹźle podpiĂ„â„˘te akcje LeadDetail, zÄąâ€šy parser potencjaÄąâ€šu, missing_item mieszany ze zwykÄąâ€šym taskiem, zlewajĂ„â€¦ce siĂ„â„˘ wiersze dziaÄąâ€šaÄąâ€ž.
...
## STAGE231H_R1C_CASE_DETAIL_COST_CORRECTION_MODAL â€” guard/test

- `scripts/check-stage231h-r1c-case-detail-cost-correction.cjs`
- `tests/stage231h-r1c-case-detail-cost-correction.test.cjs`

Zakres: wspĂłlne okno `Koryguj wpĹ‚atÄ™/koszt`, czerwone koszty, update/delete kosztu, docs/run report/Obsidian payload.
...
## STAGE231H_R1G3_CASE_DETAIL_MANUAL_UI_PASS â€” manual confirmation guard
