...
## 2026-05-16 Ă˘â‚¬â€ť Guard Stage92: calendar selected day readable actions {#STAGE92_CALENDAR_SELECTED_DAY_READABLE_ACTIONS}

Guard: `tests/stage92-calendar-selected-day-readable-actions.test.cjs`

Zakres:
- peÄąâ€šne etykiety typÄ‚Ĺ‚w wpisu,
- obecnoÄąâ€şĂ„â€ˇ `data-cf-entry-title`,
- peÄąâ€šne akcje `Edytuj`, `+1H`, `+1D`, `+1W`, `Zrobione/PrzywrÄ‚Ĺ‚Ă„â€ˇ`, `UsuÄąâ€ž`,
...
## STAGE93_GUARD_CALENDAR_WEEK_RAIL_CLEANUP Ă˘â‚¬â€ť 2026-05-16
...
- Naprawiono guard Stage93 tak, aby nie szukaÄąâ€š kruchego znacznika koÄąâ€žca po `calendar-week-plan-list`.
- Dodano lokalny sweep: `scripts/check-closeflow-calendar-ui-sweep-stage94.cjs`.
- Sweep wykrywa stare/ryzykowne wzorce Calendar UI: legacy week filter, badge count, dual selected-day render, bÄąâ€šĂ„â„˘dne ÄąĹźrÄ‚Ĺ‚dÄąâ€šo selected day, duplikaty klas i stare selektory CSS.
...
- `tests/stage96-leads-right-rail-width-position.test.cjs` dopiĂ„â„˘ty do quiet release gate.
- Guard blokuje lokalny grid 195px/300px i wymusza wspÄ‚Ĺ‚lny right rail source of truth.
...
- `tests/stage98-polish-mojibake-calendar-guard.test.cjs` Ă˘â‚¬â€ť hard fail for mojibake in `src/`, `tests/`, `scripts/`, `_project/`.
- `tests/stage99-calendar-active-class-contract.test.cjs` Ă˘â‚¬â€ť active class/CSS contract for Calendar and V9 CSS.
- `tests/stage100-calendar-week-plan-entry-visible.test.cjs` Ă˘â‚¬â€ť one active week-plan DOM/CSS model and no Stage94 V2/V3/V4 week-plan CSS markers.
- `tests/stage104-calendar-rendered-week-plan-smoke.test.cjs` Ă˘â‚¬â€ť rendered-skeleton smoke for visible title/relation/actions plus CSS non-hidden checks.
- `scripts/closeflow-release-check-quiet.cjs` Ă˘â‚¬â€ť includes Stage104 in required tests.
...
- Guard jest podpiĂ„â„˘ty do `scripts/closeflow-release-check-quiet.cjs`.
...
Decyzja: guardy UI majĂ„â€¦ sprawdzaĂ„â€ˇ efekt renderu/semi-renderu, nie tylko markery, klasy i stringi w ÄąĹźrÄ‚Ĺ‚dÄąâ€šach.

Zakres:
- fixture: Akt jaskiniowiec, 10:29, Wydarzenie, Zaplanowane, Brak powiĂ„â€¦zania,
- visible action labels: Edytuj, +1H, +1D, +1W, Zrobione, UsuÄąâ€ž,
- brak runtime markerÄ‚Ĺ‚w: ReferenceError, APP_ROUTE_RENDER_FAILED, Missing lazy page export,
- CSS selected-day V9 nie moÄąÄ˝e ukrywaĂ„â€ˇ krytycznych slotÄ‚Ĺ‚w renderu,
- usuniĂ„â„˘ty orphan selector V9 ::after, ktÄ‚Ĺ‚ry mÄ‚Ĺ‚gÄąâ€š podpinaĂ„â€ˇ V9 pod legacy hidden selected-day panel.
...
-
ode --test tests/stage113-closeflow-logo-source-contract.test.cjs pilnuje assetÄ‚Ĺ‚w i mapowania logo w Layout/Login.
...
- Cel: LeadDetail nie moÄąÄ˝e mieĂ„â€ˇ wÄąâ€šasnej wyspy `InfoLine` / `lead-detail-contact-grid`; LeadDetail i ClientDetail majĂ„â€¦ uÄąÄ˝ywaĂ„â€ˇ wspÄ‚Ĺ‚lnego `src/components/entity-contact-card.tsx`.
- Komenda: `node --test tests/stage115-lead-contact-card-client-parity.test.cjs`.
- Test rĂ„â„˘czny: /leads/:id, porÄ‚Ĺ‚wnaĂ„â€ˇ lewĂ„â€¦ kartĂ„â„˘ kontaktowĂ„â€¦ z lewĂ„â€¦ kartĂ„â€¦ klienta: telefon, e-mail, firma, ostatni kontakt, ikony, przyciski kopiowania.
...
- Purpose: overdue LeadDetail work items must show `ZalegÄąâ€še`, use red/danger styling, and remove mojibake separators.
...
- obecnoÄąâ€şĂ„â€ˇ ListDTO constants,
- cache GET 30s,
- wpis w package.json.
...
## 2026-06-04 Ă˘â‚¬â€ť Stage221 owner-control roadmap memory guard

Guard:
- `node scripts/check-stage221-owner-control-roadmap-memory.cjs`

Zakres:
- sprawdza, czy `_project/07_NEXT_STEPS.md` zawiera peÄąâ€šny blok Stage221,
- sprawdza obecnoÄąâ€şĂ„â€ˇ etapÄ‚Ĺ‚w: A35, A35B, A41, A46, A42, A45, A44, A36, A47,
- sprawdza, czy decyzja owner-control zostaÄąâ€ša dopisana do `_project/04_DECISIONS.md`,
...
Test rĂ„â„˘czny:
- PrzeczytaĂ„â€ˇ nowy blok w `_project/07_NEXT_STEPS.md` i potwierdziĂ„â€ˇ, ÄąÄ˝e kolejnoÄąâ€şĂ„â€ˇ etapÄ‚Ĺ‚w nie skacze po plikach i da siĂ„â„˘ z niej wdraÄąÄ˝aĂ„â€ˇ kolejno.
...
## STAGE226R10D2_DUPLICATE_CONFLICT_CONFIRMATION_GATE_PATCHER_FIX Ă˘â‚¬â€ť guards
...
- kontrakt: brak .catch(() => ({ candidates: [] })) przy conflict preflight; bÄąâ€šĂ„â€¦d checkerÄ‚Ĺ‚w zatrzymuje zapis; konflikt pokazuje komunikat i wymaga Ă˘â‚¬ĹľDodaj mimo toĂ˘â‚¬ĹĄ.

## STAGE226R11_GCAL_TIMEZONE_REMINDER_TRUTH Ă˘â‚¬â€ť guardy i testy
...
## STAGE226R11B_GCAL_TIMEZONE_TEST_CROSS_REALM_FIX Ă˘â‚¬â€ť guardy/testy

- data i godzina: 2026-06-06 15:05 Europe/Warsaw
- test: npm run test:stage226r11-gcal-timezone-reminder-truth
- powÄ‚Ĺ‚d: assert z node:assert/strict porÄ‚Ĺ‚wnywaÄąâ€š obiekt z VM realm do zwykÄąâ€šego obiektu i raportowaÄąâ€š false negative mimo identycznej struktury.

<!-- STAGE227A_SALES_FUNNEL_MOVEMENT_VIEW_GUARDS_START -->
## 2026-06-06 15:35 Europe/Warsaw Ă„â€šĂ‹ÂÄ‚ËĂ˘â‚¬ĹˇĂ‚Â¬Ă˘â‚¬ĹĄ STAGE227A guardy i testy

- `node scripts/check-stage227a-sales-funnel-movement-view.cjs`
- `node --test tests/stage227a-sales-funnel-movement-view.test.cjs`
- `npm run build`
- `npm run verify:closeflow:quiet`
- Regresja R10 lead/client separation, jeÄ‚â€žĂ„â€¦Ä‚ËĂ˘â€šÂ¬ÄąĹşli pliki istniejĂ„â€šĂ˘â‚¬ĹľĂ˘â‚¬Â¦.
- Regresja R11 Google Calendar timezone reminder truth, jeÄ‚â€žĂ„â€¦Ä‚ËĂ˘â€šÂ¬ÄąĹşli pliki istniejĂ„â€šĂ˘â‚¬ĹľĂ˘â‚¬Â¦.
- Manual smoke `/funnel`.
<!-- STAGE227A_SALES_FUNNEL_MOVEMENT_VIEW_GUARDS_END -->

<!-- STAGE227B_SALES_FUNNEL_DECISION_LIST_GUARDS_START -->
## 2026-06-06 15:45 Europe/Warsaw Ă„â€šĂ‹ÂÄ‚ËĂ˘â‚¬ĹˇĂ‚Â¬Ă˘â‚¬ĹĄ STAGE227B Ă„â€šĂ‹ÂÄ‚ËĂ˘â‚¬ĹˇĂ‚Â¬Ă˘â‚¬ĹĄ guardy/testy
...
## 2026-06-06 17:05 Europe/Warsaw Ă„â€šĂ‹ÂÄ‚ËĂ˘â‚¬ĹˇĂ‚Â¬Ă˘â‚¬ĹĄ STAGE228A guardy i testy
...
- Zakres: /funnel bez panelu wÄąâ€šaÄąâ€şciciela, kafelki przez OperatorMetricTile, jeden rzĂ„â€¦d na desktopie, refresh w stage actions.
<!-- /STAGE228H_R3_GUARD -->

<!-- STAGE228R1_RAIL_TASKS_PATTERN_GUARD -->
## Stage228R1 guard
- npm run check:stage228r1-rail-tasks-pattern
- Chroni klasĂ„â„˘ bÄąâ€šĂ„â„˘du: rozjazd tekstu/rytmu right rail wzglĂ„â„˘dem wzorca /tasks.
<!-- /STAGE228R1_RAIL_TASKS_PATTERN_GUARD -->
...
Chroni:
- LeadDetail: open-only Braki i blokady + RozwiĂ„â€¦ÄąÄ˝ brak.
- ClientDetail: open-only Braki i blokady + RozwiĂ„â€¦ÄąÄ˝.
- CaseDetail: istniejĂ„â€¦cy case_items status accepted/rejected i filtr accepted poza aktywnymi brakami.
...
- Guard nadal wymaga updateTaskInSupabase, status done/resolved i akcji RozwiĂ„â€¦ÄąÄ˝/RozwiĂ„â€¦ÄąÄ˝ brak.
...
- R14 guard akceptuje szczegolowy wording testu:
  - LeadDetail -> Brak -> save -> refresh -> visible -> RozwiĂ„â€¦ÄąÄ˝ brak -> refresh -> hidden
  - ClientDetail -> Brak -> save -> refresh -> visible -> RozwiĂ„â€¦ÄąÄ˝ -> refresh -> hidden
...
Kontrakt:
- debug dyktowania jest opcjonalny i domyÄąâ€şlnie wyÄąâ€šĂ„â€¦czony.
...
- `Kopiuj trace` i `WyczyÄąâ€şĂ„â€ˇ trace` sĂ„â€¦ widoczne w panelu trace.
- trace box pokazuje instrukcjĂ„â„˘ nawet gdy debug jest wyÄąâ€šĂ„â€¦czony.
...
- Quick capture uÄąÄ˝ywa lead-form-section / lead-form-textarea jako source truth formularzy.
- Tekst textarea, placeholder, caret i przyciski majĂ„â€¦ wymuszony kontrast mobilny.
...
- Rejestracja pokazuje copy o moÄąÄ˝liwoÄąâ€şci utworzenia trial/workspace dla nowego konta Google.
- /api/me ma marker obecnego public trial bootstrap przez OAuth.
- Next steps zawiera kolejkĂ„â„˘ STAGE231B/C/D/E.
<!-- STAGE231A_GOOGLE_AUTH_ENTRY_CONSISTENCY_GUARD_END -->

## STAGE231D_GOOGLE_AUTH_INTENT_GATE Ă˘â‚¬â€ť guards/tests
...
## STAGE231D_R5_GOOGLE_LOGIN_MISSING_INTENT_HARD_GATE Ă˘â‚¬â€ť guards/tests
...
## 2026-06-10 Ă˘â‚¬â€ť STAGE231B0 guard
...
## 2026-06-10 17:10 Europe/Warsaw Ă˘â‚¬â€ť STAGE231D0A Ă˘â‚¬â€ť Visual Source of Truth guard
...
Chroniona klasa bÄąâ€šĂ„â„˘du:
- lokalne style kart/kafelkÄ‚Ĺ‚w bez mapy VST,
- nowe ikony bez centralnego registry,
- nowe badgeĂ˘â‚¬â„˘e/statusy bez `StatusPill`,
- niespÄ‚Ĺ‚jne przyciski akcji bez `Button`/`entity-actions`,
- finance rows bez `FinanceMiniSummary`/finance CSS,
- powrÄ‚Ĺ‚t mojibake w dokumentach D0A,
- pominiĂ„â„˘cie roadmapy D0A przed D0.
<!-- STAGE231D0A_VISUAL_SOURCE_TRUTH_GUARDS_END -->

<!-- STAGE231D0A_R3_GUARD_PAYLOAD_EOF_RESCUE_GUARDS_START -->
## STAGE231D0A-R3 Ă˘â‚¬â€ť guard/test closeout
...
Warunek: brak FAIL, brak new blank line at EOF, brak brakujĂ„â€¦cych tokenÄ‚Ĺ‚w payloadu Obsidiana.
...
## 2026-06-10 20:05 Europe/Warsaw Ă˘â‚¬â€ť STAGE231D3-R7 guards
...
- prefiks NajbliÄąÄ˝sza akcja: w samym slocie kafelka,
- brak Sprawy:, Aktywna prowizja, Zarobione Äąâ€šĂ„â€¦cznie,
- brak osobnych slotÄ‚Ĺ‚w telefonu/e-maila i dwuwierszowego layoutu.
...
## 2026-06-12 15:00 Europe/Warsaw Ă˘â‚¬â€ť STAGE231D0F guard/test
...
Regresje powiĂ„â€¦zane:
- `node scripts/check-stage231d0d-r4-case-detail-lean-service-workspace.cjs` jeÄąâ€şli istnieje.

Guard sprawdza:
- marker `STAGE231D0F_FUNNEL_OWNER_DASHBOARD_VISUAL_ALIGNMENT`,
- istniejĂ„â€¦ce markery lejka `data-stage227b-decision-list-view` i `data-stage228a-funnel-truth-clickability`,
- wpisy UI Dictionary dla Lejka,
- filtry wÄąâ€šaÄąâ€şcicielskie,
- zasadĂ„â„˘ `lista decyzji / nie kanban`,
- brak SQL, wykresÄ‚Ĺ‚w, drag/drop i ciĂ„â„˘ÄąÄ˝kiego lokalnego stylu kafelkÄ‚Ĺ‚w.
...
## 2026-06-12 Ă˘â‚¬â€ť STAGE231D0G-CLOSEOUT Visual Tile Source Truth Atlas
...
Chroni klasĂ„â„˘ bÄąâ€šĂ„â„˘du: kafelki display-only bez CTA, brak edycji potencjaÄąâ€šu, brak widocznego potencjaÄąâ€šu przy tworzeniu leada, zlewajĂ„â€¦cy siĂ„â„˘ layout work-row i bÄąâ€šĂ„â€¦d group.key w overflow.

## 2026-06-14 10:40 Europe/Warsaw - STAGE231G R6 guard/test correction

R6 poprawia guard/test po faÄąâ€šszywym zamkniĂ„â„˘ciu R5: walidacja sprawdza realne klasy content/status/actions oraz brak stale group.key tylko w overflow section. Guard jest ASCII/no-BOM.
...
Chroni: potencjaÄąâ€š otwierany jako osobny modal tylko do wartoÄąâ€şci, zapis value + deal_value w API, brak otwierania peÄąâ€šnej edycji leada z CTA potencjaÄąâ€šu oraz desktopowe wyrÄ‚Ĺ‚wnanie akcji wiersza.
...
- klasa bĂ„Ä…Ă˘â‚¬ĹˇÄ‚â€žĂ˘â€žËdu: martwe/Ă„Ä…ÄąĹşle podpiÄ‚â€žĂ˘â€žËte akcje LeadDetail, zĂ„Ä…Ă˘â‚¬Ĺˇy parser potencjaĂ„Ä…Ă˘â‚¬Ĺˇu, missing_item mieszany ze zwykĂ„Ä…Ă˘â‚¬Ĺˇym taskiem, zlewajÄ‚â€žĂ˘â‚¬Â¦ce siÄ‚â€žĂ˘â€žË wiersze dziaĂ„Ä…Ă˘â‚¬ĹˇaĂ„Ä…Ă˘â‚¬Ĺľ.
...
## STAGE231H_R1C_CASE_DETAIL_COST_CORRECTION_MODAL Ă˘â‚¬â€ť guard/test

- `scripts/check-stage231h-r1c-case-detail-cost-correction.cjs`
- `tests/stage231h-r1c-case-detail-cost-correction.test.cjs`

Zakres: wspÄ‚Ĺ‚lne okno `Koryguj wpÄąâ€šatĂ„â„˘/koszt`, czerwone koszty, update/delete kosztu, docs/run report/Obsidian payload.
...
## STAGE231H_R1G3_CASE_DETAIL_MANUAL_UI_PASS Ă˘â‚¬â€ť manual confirmation guard
