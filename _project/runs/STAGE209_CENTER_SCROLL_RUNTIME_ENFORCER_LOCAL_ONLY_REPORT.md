# STAGE209_CENTER_SCROLL_RUNTIME_ENFORCER_LOCAL_ONLY_REPORT
...
- Runtime JSON po Stage208 pokazał:
  - body overflowY hidden i canScroll false
  - main canScroll true, ale overflowY visible
  - content canScroll true, ale overflowY visible
  - calendar jest wyższy niż viewport
- To oznacza, że właściwy element ma nadmiar treści, ale nie został właścicielem scrolla.
- Stage209 nie dokłada kolejnego zwykłego CSS-a.
- Stage209 dodaje runtime enforcer w Layout.tsx przez useEffect.
- Enforcer używa element.style.setProperty(..., important), więc wygrywa z CSS !important.
- Scroll-ownerem ma być main[data-shell-main=true] > div.view.active[data-shell-content=true].
- Skala 75% zostaje.
- Nie wykonano git add, commita ani pusha.

## WYNIK SKRYPTU

{   "layoutHasUseEffect": false,   "layoutHasStage209": true,   "appOldScrollImportsRemoved": true }
...
- node scripts/apply-stage209-center-scroll-runtime-enforcer.cjs
- node scripts/check-stage200-no-legacy-visual-scale.cjs, jeśli istnieje
- npm run build
- npm run dev
- DevTools:
  - body.canScroll=false
  - main.overflowY=hidden
  - content.overflowY=auto
  - content.canScroll=true
...
- Sprawdzić /calendar, /notifications, /activity, /ai-drafts.
- Jeżeli content.overflowY nadal będzie visible, oznacza to że useEffect się nie uruchamia albo Layout nie jest tym Layoutem.
