# STAGE197_DISABLE_OLD_SIDEBAR_FIT_CONTENT_IMPORT_LOCAL_ONLY_REPORT

FAKTY:
- Debug aplikacji pokazał: main.main height=912, aside.sidebar height=729.59375.
- Realny sidebar był krótszy o około 182 px.
- Audyt wykazał stary import: eliteflow-sidebar-user-footer-below-nav.css.
- Ten stary import jest opisany jako remove_after_stage: after sidebar component contract finalization.
- W jego okolicy audyt wykazał toksyczne reguły: align-self: start !important; height: fit-content !important; min-height: 0 !important.
- Stage195 jest już kontraktem layoutu sidebara, więc stary import został wyłączony.
- Nie wykonano git add, commita ani pusha.

DECYZJA DAMIANA:
- Ma być naprawiony realny panel boczny, nie tło za panelem.
- Nie dokładamy kolejnych ślepych CSS-plastrów.
- Usuwamy stary import, który skraca sidebar do zawartości.
...
- npm run dev
- Ctrl+F5
- Ręcznie: /, /leads, /tasks, /settings.
- Sprawdzić, czy aside.sidebar dochodzi do dołu viewportu.
- Sprawdzić, czy Trial / user / Wyloguj się są nadal widoczne i czytelne.
...
NASTĘPNY KROK:
- Jeżeli sidebar nadal ma 729px, sprawdzić computed styles dla height/max-height i znaleźć kolejną regułę !important.
