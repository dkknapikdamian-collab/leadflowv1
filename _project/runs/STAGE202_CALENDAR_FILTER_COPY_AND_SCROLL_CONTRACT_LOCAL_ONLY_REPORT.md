# STAGE202_CALENDAR_FILTER_COPY_AND_SCROLL_CONTRACT_LOCAL_ONLY_REPORT
...
- W kalendarzu tygodniowym lewy kafel pokazywał tekst: Najbliższe 7 dni / Najszybszy filtr.
- Damian zdecydował, że ten tekst ma zniknąć.
- W kalendarzu miesięcznym i innych zakładkach content może wychodzić poza widoczny panel.
- Scroll powinien być dostępny w każdej zakładce, kiedy content się nie mieści.
- Layout używa main[data-shell-main=true] oraz .view.active jako głównego kontenera treści.
- Stage202 usuwa panel-head z calendar-week-filter.
- Stage202 dodaje globalny scroll contract dla main/view/cf-html-view.
- Stage202 wzmacnia przewijanie kalendarza miesięcznego i panelu wybranego dnia.
...
- kontrola braku Najbliższe 7 dni / Najszybszy filtr w Calendar.tsx
- node scripts/check-stage200-no-legacy-visual-scale.cjs, jeśli istnieje
- npm run build
- npm run dev
- ręcznie: /calendar?view=week, /calendar?view=month, /tasks, /leads, /settings
...
- Sprawdzić tydzień: lewy kafel bez nagłówka/opisu.
- Sprawdzić miesiąc: panel wybranego dnia i dłuższe listy dają się przewinąć.
- Sprawdzić inne zakładki: jeżeli content się nie mieści, main scrolluje.
