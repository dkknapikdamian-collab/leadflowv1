# STAGE210C_FORCE_DEV_DETAIL_IDS_LOCAL_ONLY_REPORT
...
- Stage210B podmienił funkcje detail by ID i build przeszedł.
- Runtime nadal pokazał INVALID_API_RESPONSE dla klienta.
- Treść błędu zaczynała się od importu TS, czyli callApi dostał nie-JSON z lokalnego endpointu.
- Przyczyna: dev-preview detail nadal mógł być wyłączony, gdy Supabase env jest skonfigurowany.
- Stage210C wymusza dev-preview dla ID zaczynających się od dev-client, dev-lead albo dev-case w trybie DEV.
- Stage210C nie dodaje nowych rekordów testowych.
- Stage210C nie dotyka Supabase.
- Nie wykonano git add, commita ani pusha.

## WYNIK SKRYPTU

{   "ok": true,   "patched": [     "isDevPreviewDetailId",     "getDevPreviewRowById",     "fetchClientByIdFromSupabase",     "fetchLeadByIdFromSupabase",     "fetchCaseByIdFromSupabase"   ],   "behavior": "dev-* detail ids use local fixtures in DEV even when Supabase env exists",   "mode": "local-only, no git add, no commit, no push" }
...
- node scripts/apply-stage210c-force-dev-detail-ids.cjs
- node scripts/check-stage200-no-legacy-visual-scale.cjs, jeśli istnieje
- npm run build
- npm run dev
- ręcznie:
  - /clients/dev-client-1
  - /leads/dev-lead-1
  - /leads/dev-lead-3
  - /cases/dev-case-1
  - /cases/dev-case-2
...
- Obejrzeć widoki szczegółów lokalnie.
- Przed pushem zdecydować, czy Stage210C zostaje jako dev-only preview support, czy cofamy go.
