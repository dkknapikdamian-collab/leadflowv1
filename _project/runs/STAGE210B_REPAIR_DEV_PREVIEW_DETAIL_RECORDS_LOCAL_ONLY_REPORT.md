# STAGE210B_REPAIR_DEV_PREVIEW_DETAIL_RECORDS_LOCAL_ONLY_REPORT
...
- Stage210 częściowo się nie udał.
- Stage210 dodał helper getDevPreviewRowById, ale nie podmienił funkcji detail by ID.
- Build po Stage210 przeszedł, ale detail-preview nadal nie musiał działać.
- Stage210B podmienia funkcje po nazwie i granicach bloku, a nie przez kruchy regex jednej linijki.
- Nie dodano nowych danych testowych.
- Używamy istniejących dev-preview fixtures z supabase-fallback.ts.
- Nie dotykamy Supabase.
- Nie wykonano git add, commita ani pusha.

## WYNIK SKRYPTU

{   "ok": true,   "patched": [     "fetchClientByIdFromSupabase",     "fetchLeadByIdFromSupabase",     "fetchCaseByIdFromSupabase"   ],   "helper": "getDevPreviewRowById",   "mode": "local-only, no git add, no commit, no push" }
...
- node scripts/apply-stage210b-dev-preview-detail-records.cjs
...
- Obejrzeć widoki szczegółów.
- Poprawiać UI lokalnie.
- Przed pushem zdecydować, czy Stage210B zostaje jako dev-only preview support, czy cofamy go.
