...
export async function syncGoogleCalendarInboundToSupabase(input?: { forceFull?: boolean; daysBack?: number; daysForward?: number; trigger?: string }) {
  // GOOGLE_CALENDAR_STAGE10F_CLIENT_INBOUND_SYNC_HELPER
  return callApi<{ ok: boolean; importedCount: number; updatedCount: number; deletedCount: number; scannedCount: number; conflicts: Array<Record<string, unknown>> }>('/api/google-calendar?route=sync-inbound', {
    method: 'POST',
    body: JSON.stringify(input || {}),
  });
}
