...
function patchExistingTooltipQuote(source) {
  return source.replace(/setAttribute\('title'\s*,\s*label\)/g, 'setAttribute("title", label)');
}
...
function patchCalendar() {
  ...
  source = patchExistingTooltipQuote(source);
