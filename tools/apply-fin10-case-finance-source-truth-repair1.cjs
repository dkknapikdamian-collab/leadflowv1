...
const marker = 'CLOSEFLOW_FIN10_CASE_FINANCE_SOURCE_TRUTH_REPAIR1';
...
function log(msg) { console.log(`[FIN-10 REPAIR1] ${msg}`); }
...
function patchPackageJson() {
  ...
  log('package.json: FIN-10 skrypty ustawione idempotentnie');
}
...
function patchCaseSettlementPanel() {
  ...
  text = text.replace(/\n  const explicitCommissionAmount = getExplicitCommissionAmount\(record\);/g, '');

  if (!text.includes('data-case-settlement-panel="fin10"')) {
    text = text.replace(
...
function patchCaseSettlementSection() {
  ...
  if (!text.includes('data-case-settlement-section="fin10"')) {
    ...
    write(p, text);
    log('CaseSettlementSection: marker FIN-10 dodany');
  } else {
    log('CaseSettlementSection: OK');
  }
}
...
function patchCaseDetail() {
  ...
  if (start !== -1 && end !== -1 && !text.slice(start, end).includes('getCaseFinanceSourceSummary(caseData, payments)')) {
...
`;
  ...
  // FIN-10: stary marker identyfikował stary aktywny panel. Nie wolno go zostawić w UI,
  // bo guardy mają wykrywać powrót starej ścieżki liczenia.
  text = text.replace(/data-case-finance-panel/g, 'data-fin10-legacy-finance-panel-removed');

  if (!text.includes('getCaseFinanceSourceSummary(caseData, payments)')) fail('CaseDetail: lokalny helper nie deleguje do FIN-10 source');
  write(p, text);
  log('CaseDetail: helper + stary marker OK');
}

function patchLoadingReferenceGuard() {
  const p = 'scripts/check-closeflow-case-detail-loading-reference.cjs';
  if (!exists(p)) return;
  let text = read(p);
  if (text.includes("source.indexOf('<CaseSettlementPanel', componentStart);") && !text.includes("source.indexOf('<CaseSettlementSection', componentStart)")) {
    text = text.replace(
    ...
    write(p, text);
    log('check-closeflow-case-detail-loading-reference: zaakceptuje CaseSettlementSection wrapper');
  } else {
    log('check-closeflow-case-detail-loading-reference: OK');
  }
}
