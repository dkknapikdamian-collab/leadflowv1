...
- `/clients` używa teraz `OperatorSideCard` dla `dawny kafel top-client-value`.
- `/leads` używa teraz `TopValueRecordsCard` dla `Najcenniejsze leady`.
- Zachowano istniejące klasy `right-card`, `quick-list`, `lead-right-card`, `operator-top-value-card` i data-markery, żeby nie robić nowego CSS ani nie łamać stage CSS.
...
- Nie dodano klas typu `operator-right-card-new-forbidden`, `operator-top-value-card-v2-forbidden`, `operator-client-value-card-v2-forbidden`.
...
4. Sprawdzić kafel `dawny kafel top-client-value` i linki do leadów.
5. Wejść na `/leads`.
