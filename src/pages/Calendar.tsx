...
function ScheduleEntryCard({ entry, actionButtonClass, actionPendingId, caseTitle, onEdit, onShift, onShiftHours, onComplete, onDelete }: ScheduleEntryCardProps) {
  ...
  return (
    <div
      ...
      className={`calendar-entry-card cf-calendar-week-plan-entry-card ${isCompletedEntry ? 'calendar-entry-completed is-completed' : ''} ${isCompletedEntry ? 'opacity-60' : ''}`}
    >
      <div className="cf-calendar-week-plan-entry-main">
        ...
        <p className={`cf-calendar-week-plan-entry-title ${isCompletedEntry ? 'text-slate-500 line-through' : 'text-slate-900'}`} title={title} data-cf-entry-title="true">
          {title}
        </p>
...
function CalendarSelectedDayTileV9({ selectedDate, entries, actionPendingId, onEdit, onShift, onShiftHours, onComplete, onDelete }: CalendarSelectedDayTileV9Props) {
  ...
  return (
    <section data-cf-calendar-selected-day-new-tile-v9="true" aria-label="Wybrany dzieĹ„">
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      {dayEntries.length > (calendarScale === 'compact' ? 3 : 4) && (
                        <div className="calendar-more">+ {dayEntries.length - (calendarScale === 'compact' ? 3 : 4)} wiÄ™cej</div>
                      )}
                    </div>
                  </div>
                );
