...
const STYLE_ID = 'closeflow-visual-foundation-stage212b-runtime';
...
export default function VisualFoundationRuntimeStage212B() {
  useEffect(() => {
    const existing = document.getElementById(STYLE_ID);
    if (existing) existing.remove();
    const style = document.createElement('style');
    style.id = STYLE_ID;
    style.setAttribute('data-stage212f-visual-foundation-runtime', 'true');
    style.textContent = css;
    document.head.appendChild(style);
    return () => {
      const current = document.getElementById(STYLE_ID);
      if (current === style) current.remove();
    };
  }, []);
