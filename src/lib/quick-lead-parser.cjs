const QUICK_LEAD_CAPTURE_STAGE27 = 'QUICK_LEAD_CAPTURE_STAGE27_RULE_PARSER';
...
function cleanWhitespace(value) { return String(value || '').replace(/\s+/g, ' ').trim(); }
...
function detectPhone(text) {
  const match = String(text || '').match(/(?:\+48\s*)?(?:\d[\s.-]?){9,}/);
  if (!match) return '';
...
function detectContactName(text) {
  const withoutContact = String(text || '')
...
function detectNextAction(text) {
  const match = String(text || '').match(/(zadzwonić|oddzwonić|wysłać|umówić|napisać|skontaktować się|przypomnieć|sprawdzić).{0,90}/i);
...
function toLocalIsoWithoutSeconds(value) {
...
function detectDueAt(text, now = new Date()) {
  const lower = String(text || '').toLowerCase();
  const { hour, minute } = detectHour(text);
  const base = new Date(now);
  if (/\bjutro\b/.test(lower)) { base.setDate(base.getDate() + 1); return toLocalIsoWithoutSeconds(setTime(base, hour, minute)); }
  if (/\b(dzisiaj|dziś|dzis)\b/.test(lower)) return toLocalIsoWithoutSeconds(setTime(base, hour, minute));
  if (/\bpo\s*\d{1,2}\b|\bo\s*\d{1,2}\b/i.test(text)) return toLocalIsoWithoutSeconds(setTime(base, hour, minute));
  return '';
}
function parseQuickLeadNote(rawText, now = new Date()) {
...
module.exports = { QUICK_LEAD_CAPTURE_STAGE27, parseQuickLeadNote };
