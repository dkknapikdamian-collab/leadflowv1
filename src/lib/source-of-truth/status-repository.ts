...
export type StatusMeta = { value: string; label: string; source: StatusSourceKind; entity: string; readOnly?: boolean };
export type StatusRepositorySection = { entity: string; sourceFiles: readonly string[]; importedOrAdapted: string; notDuplicated: string; source: boolean; derived: boolean; uiOnly: boolean; legacy: boolean; legacyAliases: Readonly<Record<string,string>>; closedValues: readonly string[]; labels: Readonly<Record<string,string>>; tones: Readonly<Record<string,string>>; items: readonly StatusMeta[]; contract: string };
...
export const clientHealthStatus = section('client.health', [src.client], CLIENT_HEALTH_OPTIONS.map((x) => ({value:x.value,label:x.label,source:'derived',entity:'client.health',readOnly:true})), {source:false, derived:true, uiOnly:false, legacy:false}, 'clientHealth !== clientSource !== portalStatus');
export const clientSourceStatus = section('client.source', [src.client], CLIENT_SOURCE_OPTIONS.map((x) => ({value:x.value,label:x.label,source:'source',entity:'client.source'})), {source:true, derived:false, uiOnly:false, legacy:false}, 'clientHealth !== clientSource !== portalStatus');
...
export const caseLifecycleStatus = section('case.lifecycle', [src.lifecycle], make(['blocked','waiting_on_client','waiting_approval','ready_to_start','in_progress','completed','needs_next_step'], 'case.lifecycle', 'derived', true), {source:false, derived:true, uiOnly:false, legacy:false}, 'case.status !== caseLifecycle.bucket', {}, ['completed']);
...
export const missingItemStatus = section('missing-item', [src.ownerMissing, src.activity, src.case], make(['missing_item','blocking_missing_item','blocksProgress'], 'missing-item', 'source'), {source:true, derived:true, uiOnly:false, legacy:true}, 'active missing source record is task/work-item, not activity history', {missing:'missing_item'}, ['resolved','deleted','done','completed','closed','cancelled','canceled','archived']);
export const ownerControlStatus = section('owner-control', [src.ownerBaseline, src.ownerMissing], make(['critical','warning','normal'], 'owner-control', 'derived', true), {source:false, derived:true, uiOnly:false, legacy:false}, 'Owner Control is derived consumer, not source');
export const activityStatus = section('activity', [src.activity], make(['status','task','event','missing_item','payment','billing','lead_moved'], 'activity', 'ui-only'), {source:false, derived:false, uiOnly:true, legacy:false}, 'activity is UI-only timeline formatting');
export const commissionStatus = section('commission', [src.financeTypes, src.financeNormalize, src.caseFinance], make(COMMISSION_STATUSES, 'commission', 'derived', true), {source:false, derived:true, uiOnly:false, legacy:true}, 'commissionStatus = derived/read-only, not manual select', {commission_pending:'expected'}, ['paid']);
export const statusRepository = {leadStatus,clientHealthStatus,clientSourceStatus,clientPortalStatus,caseStatus,caseLifecycleStatus,taskStatus,eventStatus,paymentStatus,missingItemStatus,ownerControlStatus,activityStatus,commissionStatus} as const;
export const STATUS_REPOSITORY_SOURCE_MAP = Object.fromEntries(Object.entries(statusRepository).map(([k,v]) => [k,{entity:v.entity,sourceFile:v.sourceFiles.join(' + '),importedOrAdapted:v.importedOrAdapted,notDuplicated:v.notDuplicated,derivedUiOnlyLegacyHandling:{source:v.source,derived:v.derived,uiOnly:v.uiOnly,legacy:v.legacy,legacyAliases:v.legacyAliases}}]));
