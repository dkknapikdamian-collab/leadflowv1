...
type CaseItemInput = { id?: string; caseId: string; title?: string; description?: string; type?: string; status?: string; isRequired?: boolean; dueDate?: string | null; order?: number; response?: string | null; fileUrl?: string | null; fileName?: string | null; approvedAt?: string | null; payload?: Record<string, unknown> };
type ActivityInput = { id?: string; caseId?: string | null; leadId?: string | null; clientId?: string | null; ownerId?: string | null; actorId?: string | null; actorType?: string; eventType?: string; payload?: Record<string, unknown>; workspaceId?: string };
type MeResponse = { workspace: { id: string; ownerId?: string | null; planId?: string | null; subscriptionStatus?: string | null; trialEndsAt?: string | null; billingProvider?: string | null; providerCustomerId?: string | null; providerSubscriptionId?: string | null; nextBillingAt?: string | null; cancelAtPeriodEnd?: boolean; dailyDigestEnabled?: boolean; dailyDigestHour?: number; dailyDigestTimezone?: string | null; dailyDigestRecipientEmail?: string | null; timezone?: string | null; ownerControlWarningDays?: number; ownerControlCriticalDays?: number; ownerControlHighValueThresholdPln?: number }; profile: { id: string; fullName?: string; companyName?: string; email?: string; role?: string; isAdmin?: boolean; isAppOwner?: boolean; appRole?: string; appearanceSkin?: string; planningConflictWarningsEnabled?: boolean; browserNotificationsEnabled?: boolean; forceLogoutAfter?: string | null }; access: { hasAccess: boolean; status: string; isTrialActive: boolean; isPaidActive: boolean } };
...
function getDevPreviewData() {
  ...
  const clients = [
    { id: 'dev-client-1', name: 'Anna Kowalska', company: 'Nova Home Studio', email: 'anna@novahome.pl', phone: '+48 600 111 222', notes: 'Klient premium, długie opisy i dużo kontekstu.', archivedAt: null, primaryCaseId: 'dev-case-1' },
...
export async function deleteCaseCostFromSupabase(id: string) {
  return callApi<SupabaseInsertResult>(`/api/cases?resource=costs&id=${encodeURIComponent(id)}`, { method: 'DELETE' });
}
...
export async function fetchTasksFromSupabase(params?: TaskEventRangeParamsStage124E) {
  ...
  if (shouldUseDevNoAuthMocks()) return [];
  const normalizedTasks = await callApi<Record<string, unknown>[]>('/api/system?apiRoute=tasks' + buildTaskEventRangeQueryStage124E(params).replace('?', '&')).then(normalizeTaskListContract);
...
export async function fetchEventsFromSupabase(params?: TaskEventRangeParamsStage124E) {
  ...
  if (shouldUseDevNoAuthMocks()) return [];
  const normalizedEvents = await callApi<Record<string, unknown>[]>('/api/system?apiRoute=events' + buildTaskEventRangeQueryStage124E(params).replace('?', '&')).then(normalizeEventListContract);
  const archiveIndex = await buildCalendarParentArchiveIndexForCascade();
