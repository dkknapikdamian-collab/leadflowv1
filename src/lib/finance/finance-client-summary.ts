...
function resolveCommissionMode(cases: Array<Record<string, unknown>>, commissionAmount: number): CommissionMode {
  const explicit = cases
    .map((row) => normalizeCommissionMode(row.commissionMode ?? row.commission_mode))
    .find((mode) => mode !== 'none');
  if (explicit) return explicit;
  return commissionAmount > 0 ? 'fixed' : 'none';
}

function resolveCommissionBase(cases: Array<Record<string, unknown>>): CommissionBase {
  const row = cases.find((entry) => entry.commissionBase != null || entry.commission_base != null);
  return normalizeCommissionBase(row?.commissionBase ?? row?.commission_base);
}
...
export function buildClientFinanceSummary(input: ClientFinanceSummaryInput = {}): FinanceSummary {
  ...
  const normalizedPayments = normalizeFinancePayments(
    (Array.isArray(input.payments) ? input.payments : []) as Record<string, unknown>[],
  );
  ...
  const remainingAmount = caseRemainingAmount > 0
    ? caseRemainingAmount
    : Math.max(0, contractValue - normalizedPaidAmount);
  const commissionRemainingAmount = Math.max(0, commissionAmount - paidCommissionAmount);
  const plannedAmount = normalizedPayments
    .filter((payment) => payment.status === 'planned')
    .reduce((sum, payment) => sum + payment.amount, 0);
  const dueAmount = normalizedPayments
  ...
  return {
    ...
    remainingAmount,
    commissionMode: resolveCommissionMode(cases, commissionAmount),
    commissionBase: resolveCommissionBase(cases),
...
function relationId(row: Record<string, unknown>, camelKey: string, snakeKey: string) {
  return pickText(row, [camelKey, snakeKey], '');
}

/**
 * Compatibility adapter for callers that pass the client, its cases and a workspace-wide payment list.
 * Relation filtering happens before normalization because the finance normalizer intentionally returns
 * only domain fields and does not preserve relation identifiers.
 */
export function buildFinanceClientSummary(client: unknown, casesInput: unknown[], paymentsInput: unknown[]): FinanceSummary {
  const clientRecord = client && typeof client === 'object' ? client as Record<string, unknown> : {};
  const cases = Array.isArray(casesInput)
    ? casesInput.filter((row): row is Record<string, unknown> => Boolean(row) && typeof row === 'object')
    : [];
  const payments = Array.isArray(paymentsInput)
    ? paymentsInput.filter((row): row is Record<string, unknown> => Boolean(row) && typeof row === 'object')
    : [];
  const clientId = relationId(clientRecord, 'id', 'id');
  const caseIds = new Set(cases.map((row) => relationId(row, 'id', 'id')).filter(Boolean));
  const currency = normalizeCurrency(
    pickText(clientRecord, ['currency', 'currencyCode', 'currency_code'], '')
      || pickText(cases[0], ['currency', 'currencyCode', 'currency_code'], '')
      || pickText(payments[0], ['currency', 'currencyCode', 'currency_code'], '')
      || 'PLN',
  );

  const scopedPayments = payments.filter((payment) => {
    const paymentClientId = relationId(payment, 'clientId', 'client_id');
    const paymentCaseId = relationId(payment, 'caseId', 'case_id');
    const paymentCurrency = normalizeCurrency(
      pickText(payment, ['currency', 'currencyCode', 'currency_code'], currency),
      currency,
    );
    if (paymentCurrency !== currency) return false;
    if (!paymentClientId && !paymentCaseId) return true;
    return (Boolean(clientId) && paymentClientId === clientId)
      || (Boolean(paymentCaseId) && caseIds.has(paymentCaseId));
  });

  return buildClientFinanceSummary({
    cases,
