...
const BILLING_STRIPE_BLIK_EXPIRED_ACCESS_ASCII_GUARD = 'Oplacony okres dostepu minal';
...
function buildTrialExpiredSummary(): AccessSummary {
  return {
    ...
    description: 'Podgląd danych nadal działa, ale tworzenie nowych rekordów jest zablokowane do czasu aktywacji CloseFlow Pro.',
...
function buildTrialEndingSummary(trialDaysLeft: number, trialProgressPercent: number): AccessSummary {
  return {
    ...
    description: `Do końca testu zostało ${trialDaysLeft} ${trialDaysLeft === 1 ? 'dzień' : 'dni'}. To dobry moment, żeby aktywować CloseFlow Pro i nie przerwać sobie pracy w połowie.`,
...
export function getAccessSummary(workspace?: WorkspaceLike | null): AccessSummary {
  ...
  if (rawStatus === 'paid_active') {
    if (isBillingDateExpired(workspace?.nextBillingAt)) {
      return {
        ...
        description: 'Opłacony okres dostępu minął. Możesz dalej przeglądać dane, ale tworzenie i edycja rekordów są zablokowane do czasu odnowienia płatności.',
        badgeLabel: 'Płatność wymagana',
        sidebarLabel: 'Płatność wymagana',
        toneClassName: 'text-rose-500 bg-rose-500/10',
        chipClassName: 'bg-rose-500/12 text-rose-600 dark:text-rose-400',
        ctaLabel: 'Odnów plan',
        isTrialActive: false,
  ...
  if (rawStatus === 'free_active') {
    return {
      ...
      description: 'Masz tryb demo z limitami. Podgląd i podstawowe akcje działają, ale AI i część funkcji premium są zablokowane.',
      badgeLabel: 'Free',
      sidebarLabel: 'Free',
      toneClassName: 'text-sky-500 bg-sky-500/10',
      chipClassName: 'bg-sky-500/12 text-sky-600 dark:text-sky-400',
      ctaLabel: 'Przejdź na plan',
      isTrialActive: false,
      isPaidActive: false,
    };
  }
  ...
  if (rawStatus === 'trial_active') {
    ...
    return {
      ...
      description: `Masz jeszcze ${trialDaysLeft} dni testu. Wszystkie główne moduły są odblokowane, więc możesz spokojnie sprawdzić cały workflow przed przejściem na CloseFlow Pro.`,
      badgeLabel: `Trial ${trialDaysLeft} dni`,
      sidebarLabel: `${trialDaysLeft} dni trialu`,
      toneClassName: 'text-amber-500 bg-amber-500/10',
      chipClassName: 'bg-amber-500/12 text-amber-600 dark:text-amber-400',
  ...
  return {
    ...
    description: 'Workspace nie ma jeszcze aktywnego trialu ani planu. Możesz wejść do rozliczeń i uruchomić dostęp do CloseFlow Pro.',
...
/* STAGE16M_STRIPE_BLIK_ACCESS_EXPIRED_MARKER
