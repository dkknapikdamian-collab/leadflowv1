...
export default async function handler(req: any, res: any) {
  try {
    ...
    if (req.method === 'POST' && (action === 'inbound-sync' || action === 'sync-from-google')) {
      // GOOGLE_CALENDAR_STAGE10G_INBOUND_HANDLER_ROUTE
      await assertWorkspaceWriteAccess(workspaceId, req);
      const result = await syncInboundGoogleCalendarToCloseFlow({
        workspaceId,
        userId,
        timeMin: typeof body.timeMin === 'string' ? body.timeMin : typeof req.query?.timeMin === 'string' ? req.query.timeMin : undefined,
        timeMax: typeof body.timeMax === 'string' ? body.timeMax : typeof req.query?.timeMax === 'string' ? req.query.timeMax : undefined,
        updatedMin: typeof body.updatedMin === 'string' ? body.updatedMin : typeof req.query?.updatedMin === 'string' ? req.query.updatedMin : undefined,
      });
